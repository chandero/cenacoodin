...
CCaptureModule _Module;

BEGIN_OBJECT_MAP(ObjectMap)
END_OBJECT_MAP()

LONG CCaptureModule::Unlock()
{
	AfxOleUnlockApp();
	return 0;
}

LONG CCaptureModule::Lock()
{
	AfxOleLockApp();
	return 1;
}
LPCTSTR CCaptureModule::FindOneOf(LPCTSTR p1, LPCTSTR p2)
{
	while (*p1 != NULL)
	{
		LPCTSTR p = p2;
...
BOOL CCaptureApp::InitATL()
{
	...
	while (lpszToken != NULL)
	{
		...
		{
			_Module.UpdateRegistryFromResource(IDR_CAPTURE, FALSE);
		...
		{
			_Module.UpdateRegistryFromResource(IDR_CAPTURE, TRUE);
