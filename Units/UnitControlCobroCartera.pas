...
var Fecha:TDate;
    ...
    _fFechaHoy :TDate;
begin
{        if EdDiasIni.Value < 10 then
        begin
          ShowMessage('El campo Desde no puede ser menor que 10');
          Exit;
        end;
        if EdDiasFin.Value < 10 then
        begin
          ShowMessage('El campo Hasta no puede ser menor que 10');
          Exit;
        end;}
        CdCarta.CancelUpdates;
        lIni.Caption := FormatDateTime('hh:mm:ss',Time);
        _fFechaHoy := fFechaActual;
...
{                Fecha := FieldByName('FECHA_INTERES').AsDateTime;
                ...
                if FieldByName('SALDO').AsCurrency > 0 then
                  ...
                  Dias := ObtenerDiasMoraCausacion(FieldByName('ID_ESTADO_COLOCACION').AsInteger,
                                                     ...
                                                     FieldByName('FECHA_DESEMBOLSO').AsDateTime,
                                                     FieldByName('FECHA_INTERES').AsDateTime,
                                                     _fFechaHoy,
                                                     FieldByName('INTERES').AsString )
                ...
                begin
                   ...
                   begin
                     ...
                     CdCarta.FieldByName('MUNICIPIO').AsString := IbDireccion.FieldByName('MUNICIPIO').AsString;
                     CdCarta.Post;
                   end;
        ...
        lFin.Caption := FormatDateTime('hh:mm:ss',Time);
    Screen.Cursor := Save_Cursor;
end;
...
//         RGEstado.SetFocus;
         ...
         FechaFinal := fFechaActual;//EncodeDate(YearOf(FechaHoy),12,31);
end;
...
begin
  with IBSQL2 do begin
     ...
     SQL.Add('Select SALDO_DISPONIBLE from SALDO_DISPONIBLE (:ID_AGENCIA,:ID_TIPO_CAPTACION,:NUMERO_CUENTA,:DIGITO_CUENTA,:ANO,:FECHA1,:FECHA2)');
     ParamByName('ID_AGENCIA').AsInteger := Agencia;
     ParamByName('ID_TIPO_CAPTACION').AsInteger := 2;
...
begin
   with IBSQL2 do begin
     ...
     SQL.Clear;
     SQL.Add('Select SALDO_DISPONIBLE from SALDO_DISPONIBLE (:ID_AGENCIA,:ID_TIPO_CAPTACION,:NUMERO_CUENTA,:DIGITO_CUENTA,:ANO,:FECHA1,:FECHA2)');
     //   SQL.Add('Select SALDO_DISPONIBLE from SALDO_DISPONIBLE_MES (:ID_AGENCIA,:ID_TIPO_CAPTACION,:NUMERO_CUENTA,:DIGITO_CUENTA,:ANO,:FECHA1,:FECHA2)');
     ParamByName('ID_AGENCIA').AsInteger := Agencia;
     ParamByName('ID_TIPO_CAPTACION').AsInteger := 1;
     ParamByName('NUMERO_CUENTA').AsInteger := Cuenta;
     ParamByName('DIGITO_CUENTA').AsInteger := Digito;
...
begin
        ...
        begin
          ...
          if (Municipio = 'OCAÑA') or (Municipio = 'OCANA') then
             Municipio := 'ocaña';
          direccion_col := LowerCase(Municipio) + ', '+ LowerCase(FieldByName('DIRECCION').AsString);
          Close;
          SQL.Clear;
          ...
          if Agencia = 1 then
             ...
             ciudad_col[1] := Upcase(ciudad_col[1]);
          Close;
        end;
...
begin
        ...
        if Role = 'CARTERA' then
        begin
           chCarta.Enabled := True;
           MasivoCartas1.Enabled := True;
        end
        else
        begin
           chCarta.Enabled := False;
           MasivoCartas1.Enabled := False;
        end;
end;
...
begin
        ...
        //IBDataSet1.ParamByName('FECHA_OBSERVACION').AsDate := fFechaActual;
        //FrmImpresion := TFrmImpresion.Create(self);
...
begin
        ...
        begin
          ...
          begin
             ...
             FrmGeneraCarta.CdCartaReal.FieldByName('MUNICIPIO').AsString := CdCarta.FieldByName('MUNICIPIO').AsString;
            ...
            begin
              ...
              begin
                ...
                FrmGeneraCarta.CdCartaReal.FieldByName('MUNICIPIO').AsString := IBDatoCodeudor.FieldByName('MUNICIPIO').AsString;                
          ...
          begin
            ...
            Open;
            if Agencia = 1 then
               FrmGeneraCarta._sCiudad := Ciudad + ' ' + FormatDateTime('dd-mmm-yyyy',Date)
            else
               FrmGeneraCarta._sCiudad := LowerCase(FieldByName('DESCRIPCION_AGENCIA').AsString) + ', '+ FormatDateTime('dd-mmm-yyyy',Date);
            Close;
          end;
          JvProgreso.Close;
