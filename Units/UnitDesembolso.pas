        ...
        begin
          ...
          begin
            if Transaction.InTransaction then
               Transaction.Commit;
            Transaction.StartTransaction;
            // VERIFICAR MONTOS DE LA NUEVA LINEA DE CREDITO
            Close;
            SQL.Clear;
            SQL.Add('SELECT ');
            ...
            SQL.Add('"col$colocacion".ID_LINEA IN (14,15)');
            ParamByName('ID_PERSONA').AsString := EdNumeroIdentificacion.Text;
            ParamByName('ID_IDENTIFICACION').AsInteger := CBtiposid.KeyValue;
            ExecQuery;
            _cValorLineaSocial := FieldByName('SALDO').AsCurrency; //valor acumulado en Nueva Linea Social y Nueva Linea esperada
            Close;
            SQL.Clear;
            SQL.Add('SELECT MAX(MONTO_MAX) AS MONTO');
            SQL.Add('FROM "col$validalinea"');
            SQL.Add('WHERE ID_LINEA = :ID_LINEA');
            ParamByName('ID_LINEA').AsInteger := 15;
            ExecQuery;
            _cMaxLinea := FieldByName('MONTO').AsCurrency;
            // FIN VERIFICA NUEVA LINEA DE CREDITOS
            Close;
            SQL.Clear;
...
{            if vLinea = 15 then
            begin
              if (_cValorLineaSocial + vDesembolso) > (_cMaxLinea) then
              begin
                ShowMessage(
              end;
            end;}
            ...
            Close;
            SQL.Clear;
            SQL.Add('select VALOR_MINIMO from "gen$minimos" where ID_MINIMO = 48');
            ExecQuery;
            reciprocidad := FieldByName('VALOR_MINIMO').AsFloat;
            Close;
            ...
            begin
               ...
               CDgarantiareal.FieldValues['id_persona'] := FieldByName('ID_PERSONA').AsString;
               CDgarantiareal.FieldValues['id_identificacion'] := FieldByName('ID_IDENTIFICACION').AsInteger;
               IBcdat.Close;
               IBcdat.SQL.Clear;
               IBcdat.SQL.Add('select * from "col$datogarantia" where MATRICULA = :MATRICULA and ID_AGENCIA = :ID_AGENCIA');
               IBcdat.ParamByName('MATRICULA').AsString := FieldByName('MATRICULA').AsString;
               IBcdat.ParamByName('ID_AGENCIA').AsInteger := Agencia;
               IBcdat.ExecQuery;
               if IBcdat.RecordCount > 0 then
               begin
                 CDgarantiareal.FieldValues['avaluo'] := IBcdat.FieldByName('AVALUO').AsCurrency;
                 CDgarantiareal.FieldValues['cuentas_orden'] := IBcdat.FieldByName('CUENTAS_DE_ORDEN').AsString;
                 CDgarantiareal.FieldValues['poliza_incendio'] := IBcdat.FieldByName('POLIZA_INCENDIO').AsString;
                 CDgarantiareal.FieldValues['valor_asegurado'] := IBcdat.FieldByName('VALOR_ASEGURADO').AsCurrency;
                 CDgarantiareal.FieldValues['fecha_registro'] := IBcdat.FieldByName('FECHA_MATRICULA').AsDate;
                 CDgarantiareal.FieldValues['fecha_ini_poliza'] := IBcdat.FieldByName('FECHA_INICIAL_POLIZA').AsDate;
                 CDgarantiareal.FieldValues['fecha_fin_poliza'] := IBcdat.FieldByName('FECHA_FINAL_POLIZA').AsDate;
                 CDgarantiareal.FieldValues['fecha_avaluo'] := IBcdat.FieldByName('FECHA_AVALUO').AsDate;
               end;
               CDgarantiareal.Post;
               Next;
            end;
...
///inicio 4 * mil
                 ...
                 AF^.CodigoPuc   := '523050000000000000'{YA ES NIIF};
                 ...
                 New(AF);
                 AF^.CuotaNumero := cuota;
                 AF^.CodigoPuc   := '243005000000000000'{YA ES NIIF};
                 AF^.FechaInicial := FechaHoy;
                 AF^.FechaFinal   := FechaHoy;
                 AF^.Dias         := 0;
                 AF^.Tasa         := 0;
...
begin
        ...
        if ParName = 'textoagencia' then
           ParValue := ParametroRep(1);
        if ParName = 'nit' then
           ParValue := Nit;
end;
...
var
   _tFrfpict1,_tFrfpict2 : TfrPictureView;
   _sRutaLogo :string;
begin
        ...
        begin
          ...
          while not Eof do
          begin
          case FieldByName('ID_HORARIO').AsInteger of
            1 : begin
                  frReport1.Dictionary.Variables.Variable['ho1'] := QuotedStr(FieldByName('HORARIO').AsString);
                  frReport1.Dictionary.Variables.Variable['diao1'] := QuotedStr(FieldByName('DIAS').AsString);
                end;
            2 : begin
                  frReport1.Dictionary.Variables.Variable['ho2'] := QuotedStr(FieldByName('HORARIO').AsString);
                  frReport1.Dictionary.Variables.Variable['diao2'] := QuotedStr(FieldByName('DIAS').AsString);
                end;
            3 : begin
                  frReport1.Dictionary.Variables.Variable['ho3'] := QuotedStr(FieldByName('HORARIO').AsString);
                  frReport1.Dictionary.Variables.Variable['diao3'] := QuotedStr(FieldByName('DIAS').AsString);
                end;
            4 : begin
                  frReport1.Dictionary.Variables.Variable['hs1'] := QuotedStr(FieldByName('HORARIO').AsString);
                  frReport1.Dictionary.Variables.Variable['diaser1'] := QuotedStr(FieldByName('DIAS').AsString);
                end;
            5 : begin
                  frReport1.Dictionary.Variables.Variable['hs2'] := QuotedStr(FieldByName('HORARIO').AsString);
                  frReport1.Dictionary.Variables.Variable['diaser2'] := QuotedStr(FieldByName('DIAS').AsString);
                end;
            end;
          Next;
          end;
        ...
        _tFrfpict2 := TfrPictureView(frReport1.FindObject('picture2'));
        _sRutaLogo := frmMain.wpath + 'logo\logo.jpg';
        if Assigned(_tFrfPict1) then
        begin
...
begin
        ...
        begin
           ...
           //Quitar proceso cuando se actualice el proceso de causaci?n
{           if CDgarantiareal.RecordCount > 0 then
           begin
            ...
            try
              if GrabarColocacion then
               begin
                 ...
                 Grabar_Garantias;//Ojo Quitar con el nuevo proceso...
                 GrabarGarantiaReal;
                 Contabilizar;
               end;
            except
...
begin
        ...
        begin
          ...
          EDAvaluo.Value := CDgarantiarealavaluo.Value;
          EdFechaAvaluo.DateTime := Date;
          EDcuentas.Value := CDgarantiarealcuentas_orden.CurValue;
          _sIdPersona := CDgarantiarealID_PERSONA.Value;
          try
            EdFechaIPoliza.Date := CDgarantiareal.FieldByName('fecha_ini_poliza').AsDateTime;
          except
            EdFechaIPoliza.Date := Date;
          end;
          try
            EdFechaFPoliza.Date := CDgarantiarealfecha_fin_poliza.Value;
          except
            EdFechaFPoliza.Date := Date;
          end;
          EDcuentas.Value := CDgarantiarealcuentas_orden.Value;
          EDvalor.Value := CDgarantiarealvalor_asegurado.Value;
          EdPoliza.Text := CDgarantiarealpoliza_incendio.Value;
          ShowModal;
        end;
        end;
end;
...
procedure TFrmDesembolso.RevisaHonorarios;
begin
...
procedure TFrmDesembolso.InfDesembolso;
begin
        with DmSolicitud.IBsolicitud3 do
        begin
          if Transaction.InTransaction then
...
function TFrmDesembolso.vFianza(vId_Colocacion: string): boolean;
begin
        Result := False;
        with IBSQL1 do
        begin
...
procedure TFrmDesembolso.BTBancoldexClick(Sender: TObject);
begin
        LimpiaInformacion;
...
function TFrmDesembolso.FianzaDescuento(vId_Colocaicon: string): boolean;
begin
        with IBSQL1 do
        begin
          if Transaction.InTransaction then
...
procedure TFrmDesembolso.BTfngClick(Sender: TObject);
begin
        LimpiaInformacion;
...
procedure TFrmDesembolso.GrabarGarantiaReal;
begin
        with dmColocacion.IBQuery do
        begin
           Close;
           ...
           begin
             ...
             begin
                Close;
                SQL.Clear;
                SQL.Add('select * from "col$datogarantia" where MATRICULA = :MATRICULA');
                ParamByname('MATRICULA').AsString := CDgarantiareal.FieldValues['matricula'];
                Open;
                //INICIO INSERT O UPDATE DE LA TABLA
                if RecordCount = 0 then
                begin
                  SQL.Clear;
                  SQL.Add('INSERT INTO');
                  ...
                  SQL.Add('ID_AGENCIA,');
                  SQL.Add('MATRICULA,');
                  SQL.Add('FECHA_MATRICULA,');
                  SQL.Add('NUMERO_ESCRITURA,');
                  SQL.Add('FECHA_ESCRITURA,');
                  SQL.Add('NOMBRE_NOTARIA,');
                  SQL.Add('CIUDAD_ESCRITURA,');
                  SQL.Add('AVALUO,');
                  SQL.Add('FECHA_AVALUO,');
                  SQL.Add('CUENTAS_DE_ORDEN,');
                  SQL.Add('POLIZA_INCENDIO,');
                  SQL.Add('VALOR_ASEGURADO,');
                  SQL.Add('FECHA_INICIAL_POLIZA,');
                  SQL.Add('FECHA_FINAL_POLIZA,');
                  SQL.Add('CODIGO_ASEGURADORA,');
                  SQL.Add('ESTADO,MODELO_VEHICULO)');
                  SQL.Add('VALUES(');
                  SQL.Add(':ID_AGENCIA,');
                  SQL.Add(':MATRICULA,');
                  SQL.Add(':FECHA_MATRICULA,');
                ...
                begin
                  ...
                  SQL.Add('"col$datogarantia"');
                  SQL.Add('SET');
                  SQL.Add('FECHA_MATRICULA = :FECHA_MATRICULA,');
                  SQL.Add('NUMERO_ESCRITURA = :NUMERO_ESCRITURA,');
                  SQL.Add('FECHA_ESCRITURA = :FECHA_ESCRITURA,');
                  SQL.Add('NOMBRE_NOTARIA = :NOMBRE_NOTARIA,');
                  SQL.Add('CIUDAD_ESCRITURA = :CIUDAD_ESCRITURA,');
                  ...
                  SQL.Add('MODELO_VEHICULO = :MODELO_VEHICULO');
                  SQL.Add('WHERE');
                  SQL.Add('ID_AGENCIA = :ID_AGENCIA AND');
                ...
                ParamByName('MATRICULA').AsString := CDgarantiareal.FieldValues['matricula'];
                try
                  ParamByName('FECHA_MATRICULA').AsDate := CDgarantiareal.FieldValues['fecha_registro'];
                except
                  ParamByName('FECHA_MATRICULA').Clear;
                end;
                ParamByName('NUMERO_ESCRITURA').AsString := CDgarantiareal.FieldValues['numero_escritura'];
                try
                  ParamByName('FECHA_ESCRITURA').AsDate := CDgarantiareal.FieldValues['fecha_escritura'];
                ...
                ParamByName('MODELO_VEHICULO').AsInteger := 0;//Temporalmente para cambios en pignoraci?n
                try
                  ExecSQL;
                except
                  MessageDlg('Error insertando en col$garantiasreal',mtError,[mbok],0);
                end;
                // FIN DEL UPDATE O EL INSERT DE LA TABLA
                // AGREGAR LOS DUE?OS DE LA GARANTIA
                Close;
                SQL.Clear;
                SQL.Add('DELETE FROM "col$garantiapersona" WHERE MATRICULA = :MATRICULA');
                ParamByName('MATRICULA').AsString := CDgarantiareal.FieldValues['matricula'];
                ExecSQL;
                Close;
                SQL.Clear;
                SQL.Add('SELECT ');
                SQL.Add('"gen$bienesraices".ID_IDENTIFICACION,');
                SQL.Add('"gen$bienesraices".ID_PERSONA');
                SQL.Add('FROM');
                SQL.Add('"gen$bienesraices"');
                SQL.Add('WHERE');
                SQL.Add('"gen$bienesraices".ID_SOLICITUD = :ID_SOLICITUD AND ');
                SQL.Add('"gen$bienesraices".MATRICULA = :MATRICULA');
                ParamByName('MATRICULA').AsString := CDgarantiareal.FieldValues['matricula'];
                ParamByName('ID_SOLICITUD').AsString := vIdSolicitud;
                Open;
                while not Eof do
                begin
                  IBQuery3.Close;
                  IBQuery3.SQL.Clear;
                  IBQuery3.SQL.Add('INSERT INTO');
                  IBQuery3.SQL.Add('"col$garantiapersona"(');
                  IBQuery3.SQL.Add('MATRICULA,');
                  IBQuery3.SQL.Add('ID_IDENTIFICACION,');
                  IBQuery3.SQL.Add('ID_PERSONA,');
                  IBQuery3.SQL.Add('ID_AGENCIA)');
                  IBQuery3.SQL.Add('VALUES(');
                  IBQuery3.SQL.Add(':MATRICULA,');
                  IBQuery3.SQL.Add(':ID_IDENTIFICACION,');
                  IBQuery3.SQL.Add(':ID_PERSONA,');
                  IBQuery3.SQL.Add(':ID_AGENCIA)');
                  IBQuery3.ParamByName('MATRICULA').AsString := CDgarantiareal.FieldValues['matricula'];
                  IBQuery3.ParamByName('ID_PERSONA').AsString := FieldByName('ID_PERSONA').AsString;
                  IBQuery3.ParamByName('ID_IDENTIFICACION').AsInteger := FieldByName('ID_IDENTIFICACION').AsInteger;
                  IBQuery3.ParamByName('ID_AGENCIA').AsInteger := Agencia;
                  IBQuery3.ExecSQL;
                  Next;
                end;
                // FIN AGREGAR DUE?OS DE LA GARANTIA
                // AGREGAR GARANTIA A LA COLOCACION
                Close;
                SQl.Clear;
                SQL.Add('INSERT INTO');
                SQL.Add('"col$garantiacol"(');
                SQL.Add('ID_AGENCIA,');
                SQL.Add('MATRICULA,');
                SQL.Add('ID_COLOCACION,TIPO_RESPALDO)');
                SQL.Add('VALUES(');
                SQL.Add(':ID_AGENCIA,');
                SQL.Add(':MATRICULA,');
                SQL.Add(':ID_COLOCACION,:TIPO_RESPALDO)');
                ...
                ParamByname('ID_AGENCIA').AsInteger := Agencia;
                ParamByName('MATRICULA').AsString := CDgarantiareal.FieldValues['matricula'];
                if EdNumeroIdentificacion.Text = CDgarantiareal.FieldValues['ID_PERSONA'] then
                   ParamByName('TIPO_RESPALDO').AsString := 'D'
                else
                   ParamByName('TIPO_RESPALDO').AsString := 'F';
                ExecSQL;
                // FIN AGREGAR GARANTIA A LA COLOCACION
                // ACTIVAR DE NUEVO LA GARANTIA
                Close;
                SQL.Clear;
                SQL.Add('update "col$datogarantia" set ESTADO = 0');
                SQL.Add('where MATRICULA = :MATRICULA');
                ParamByName('MATRICULA').AsString := CDgarantiareal.FieldValues['matricula'];
                ExecSQL;
                // FIN ACTUALIZAR GARANTIA
                //AGREGAR CONTROL DE LA GARANTIA
                Close;
                SQL.Clear;
                SQL.Add('INSERT INTO');
                SQL.Add('"col$controlgarantia"(');
                SQL.Add('ID_AGENCIA,');
                SQL.Add('MATRICULA,');
                SQL.Add('ESTADO,');
                SQL.Add('FECHA,');
