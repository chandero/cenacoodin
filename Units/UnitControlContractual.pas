            ...
            begin
              ...
              begin
               ...
               //***********
                  ...
                  CdContractual.FieldValues['IDPERSONA'] := vIdPersona;
                  CdContractual.FieldValues['IDIDENTIFICACION'] := vId;
...
begin
        ...
        begin
          ...
          begin
            SQL.Add('and ("cap$maestro".FECHA_VENCIMIENTO <= :FECHA)');
            SQL.Add(' AND("cap$maestro".FECHA_VENCIMIENTO_PRORROGA IS NULL)');
            SQL.Add('UNION');
            SQL.Add('SELECT');
            SQL.Add('"gen$persona".NOMBRE,');
            SQL.Add('"gen$persona".PRIMER_APELLIDO,');
            ...
            SQL.Add('WHERE');
            SQL.Add('("cap$maestrotitular".ID_TIPO_CAPTACION = 5) AND ("cap$maestro".ID_ESTADO IN (1,6))');
            SQL.Add('and ("cap$maestro".FECHA_VENCIMIENTO_PRORROGA <= :FECHA)');
            SQL.Add(' AND("cap$maestro".FECHA_VENCIMIENTO_PRORROGA IS NOT NULL)');
          ...
          begin
            ...
            begin
                vId := FieldByName('ID_IDENTIFICACION').AsInteger;
                vIdPersona := FieldByName('ID_PERSONA').AsString;
                //Validar fecha de finalización del Contractual
                CdContractual.Append;
                CdContractual.FieldValues['NUMERO'] := FieldByName('NUMERO_CUENTA').AsInteger;
                CdContractual.FieldValues['DG'] := FieldByName('DIGITO_CUENTA').AsInteger;
                CdContractual.FieldValues['FECHA_APERTURA'] := FieldByName('FECHA_APERTURA').AsDateTime;
                ...
                CdContractual.FieldValues['DIAS'] := 0;
                CdContractual.FieldValues['IDPERSONA'] := IBQuery1.FieldByName('ID_PERSONA').AsString;
                CdContractual.FieldValues['IDIDENTIFICACION'] := IBQuery1.FieldByName('ID_IDENTIFICACION').AsString;
                CdContractual.Post;
              //***********
...
begin
        ...
        begin
           ...
           frmDireccionesAsociado.Free;
        end;

end;

procedure TFrmControlContractual.BitBtn6Click(Sender: TObject);
var
        _vDias :Currency;
begin
        with IBQuery2 do
        begin
          ...
          while not Eof do
          begin
            _vDias := Int(FieldByName('DIAS').AsInteger / 30);
            if FieldByName('C_VENCIDA').AsCurrency <> _vDias then
            begin
                IBQuery3.ParamByName('CUOTA').AsCurrency := _vDias;
                IBQuery3.ParamByName('NM').AsInteger := FieldByName('NUMERO_CUENTA').AsInteger;
                IBQuery3.ParamByName('DG').AsInteger := FieldByName('DIGITO_CONTROL').AsInteger;
                IBQuery3.ParamByName('FECHA').AsDateTime := FieldByName('FECHA_ACTUAL').AsDateTime;
                IBQuery3.ExecSQL;
            end;
            Next;
          end;
