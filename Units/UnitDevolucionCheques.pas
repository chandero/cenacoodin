        ...
        if (AColumn = 4) then begin
          ...
          try
            ...
            if IBSQL2.RecordCount <= 0 then begin
             MessageDlg('El Cheque no est? Registrado',mtError,[mbcancel],0);
             SGCheques.Cells[AColumn,ARow]:= '';
             AR^.Cheque := '';
             //Exit;
            end
...
// Codigo Cuentas por Cobrar
             ...
             with IBComprobante do begin
                ...
                ParamByName('DESCRIPCION').AsString := 'Devoluci?n' + Banco + 'de Cheque No.' + ' ' + AR^.Cheque +  ' ' + AR^.DeBanco + ' ' + AR^.DeTipo +  ' ' + 'No.' + ' ' + IntToStr(AR^.Cuenta);               //Format(Descripcion,[AR^.DeBanco,AR^.IdTipo,AR^.IdAgencia,AR^.Cuenta])+ ', Fecha:' + DateToStr(AR^.Fecha);
                ...
                if (SaldoAhorros < AR^.Valor) then begin
                  ...
                  if (AR^.Valor > 0) then
                   begin
                     ...
                     ParamByName('DESCRIPCION_MOVIMIENTO').AsString := 'Devoluci?n de Cheque No.' + ' ' + AR^.Cheque + ' ' + AR^.DeBanco;
