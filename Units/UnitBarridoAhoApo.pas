...
var Total:Integer;
    ...
    _cSaldo :Currency;
begin
        _dFechaInicial := EncodeDate(YearOf(fFechaActual),01,01);
        _dFechaFinal := fFechaActual;
        _iAno :=  YearOf(fFechaActual);
        //ShowMessage(DateToStr(_dFechaInicial) + ' ' + DateToStr(_dFechaFinal));
        ...
        with IBQuery1 do begin
         ...
         try
          ...
          try
              ...
              frmProgreso.Cerrar;
              MessageDlg('No se pudo crear la tabla temporal para la liquidaci�n',mtError,[mbcancel],0);
              Exit;
          end;
         ...
         while not Eof do begin
           ...
           frmProgreso.InfoLabel := 'Analizando Aportaci�n:'+FieldByName('ID_TIPO_CAPTACION').AsString + Format('%.2d',[FieldByName('ID_AGENCIA').AsInteger]) + '-' + Format('%.7d',[FieldByName('NUMERO_CUENTA').AsInteger]) + '-' + FieldByName('DIGITO_CUENTA').AsString;
           Application.ProcessMessages;
           //Antiguo Proceso
           {IBQuery2.Close;
           IBQuery2.SQL.Clear;
           ...
           IBQuery2.Close;
           IBQuery2.SQL.Clear;
           IBQuery2.SQL.Add('SELECT SALDO_ACTUAL FROM SALDO_ACTUAL_MES(:AG,:TP,:CTA,:DG,:ANO,:FECHA1,:FECHA2)');
           IBQuery2.ParamByName('AG').AsInteger := FieldByName('ID_AGENCIA').AsInteger;
           IBQuery2.ParamByName('TP').AsInteger := FieldByName('ID_TIPO_CAPTACION').AsInteger;
           IBQuery2.ParamByName('CTA').AsInteger := FieldByName('NUMERO_CUENTA').AsInteger;
           IBQuery2.ParamByName('DG').AsInteger := FieldByName('DIGITO_CUENTA').AsInteger;
           IBQuery2.ParamByName('ANO').AsInteger := _iAno;
           IBQuery2.ParamByName('FECHA1').AsDate := _dFechaInicial;
           IBQuery2.ParamByName('FECHA2').AsDate := _dFechaFinal;
           IBQuery2.Open;
           SaldoCuentaApo := IBQuery2.FieldByName('SALDO_ACTUAL').AsCurrency;
           ...
           if SaldoCuentaApo < SaldoMinimoApo then
            begin
                 ...
                 IBQuery2.SQL.Add('SELECT SALDO_ACTUAL FROM SALDO_ACTUAL_MES(:AG,:TP,:CTA,:DG,:ANO,:FECHA1,:FECHA2)');
                 IBQuery2.ParamByName('AG').AsInteger := IBQuery1.FieldByName('ID_AGENCIA').AsInteger;
                 IBQuery2.ParamByName('TP').AsInteger := 2;
                 IBQuery2.ParamByName('CTA').AsInteger := IBQuery1.FieldByName('NUMERO_CUENTA').AsInteger;
                 IBQuery2.ParamByName('DG').AsInteger := StrToInt(digitocontrol(2,Format('%.7d',[IBQuery1.FieldByName('NUMERO_CUENTA').AsInteger])));
                 IBQuery2.ParamByName('FECHA1').AsDate := _dFechaInicial;
                 IBQuery2.ParamByName('FECHA2').AsDate := _dFechaFinal;
                 IBQuery2.ParamByName('ANO').AsInteger := _iAno;
                 IBQuery2.Open;
                 SaldoCuentaAho := IBQuery2.FieldByName('SALDO_ACTUAL').AsCurrency;
                 //SaldoCuentaAho := SaldoCuentaAho - SaldoMinimoAho;
                 if SaldoCuentaAho > 0 then
                  begin
                   _cSaldo := SaldoCuentaAho - DifMin;
                   if _cSaldo < 0 then
                      DifMin := SaldoCuentaAho;
                   IBQuery2.Close;
                   IBQuery2.SQL.Clear;
       ...
       with IBQuery1 do begin
         ...
         while not Eof do begin
             ...
             Application.ProcessMessages;
              ...
              IBQuery2.ParamByName('DESCRIPCION_MOVIMIENTO').AsString := 'Actualizaci�n Aportes Sociales';
