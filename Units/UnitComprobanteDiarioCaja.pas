...
type
  TfrmComprobanteDiario = class(TForm)
    ...
    cdsMovsID_AGENCIA: TIntegerField;
    cdsMovsAGENCIA: TStringField;
    cdsMovsID_CAJA: TIntegerField;
    cdsMovsFECHA_MOV: TDateTimeField;
    cdsMovsORIGEN_MOVIMIENTO: TIntegerField;
    cdsMovsID_TIPO_CAPTACION: TIntegerField;
    cdsMovsNUMERO_CUENTA: TIntegerField;
    cdsMovsNOMBRE: TStringField;
    cdsMovsDOCUMENTO: TStringField;
    cdsMovsID_TIPO_MOVIMIENTO: TIntegerField;
    cdsMovsEFECTIVO_DB: TCurrencyField;
    cdsMovsCHEQUE_DB: TCurrencyField;
    cdsMovsEFECTIVO_CR: TCurrencyField;
    cdsMovsCHEQUE_CR: TCurrencyField;
    IBQRecaudo: TIBQuery;
    IBQRecaudoID_AGENCIA: TSmallintField;
    IBQRecaudoID_CAJA: TSmallintField;
    IBQRecaudoID_RECAUDO: TSmallintField;
    IBQRecaudoDESCRIPCION_RECAUDO: TIBStringField;
    IBQRecaudoCODIGO_RECAUDO: TIBStringField;
    IBQRecaudoID_TIPO_CAPTACION: TIntegerField;
    IBQRecaudoNUMERO_CUENTA: TIntegerField;
    IBQRecaudoEFECTIVO: TIBBCDField;
    IBSQLconsec: TIBSQL;
...
// Buscar Consecutivo
        ...
        _lista.Add(IntToStr(Consecutivo));
        ...
        begin
            ...
            try
             ...
             begin
               ...
               begin
                 cdsMovs.Insert;
                 cdsMovsID_AGENCIA.Value := FieldByName('ID_AGENCIA').AsInteger;
                 cdsMovsAGENCIA.Value := FieldByName('DESCRIPCION_AGENCIA').AsString;
                 cdsMovsID_CAJA.Value := FieldByName('ID_CAJA').AsInteger;
                 cdsMovsFECHA_MOV.Value := FieldByName('FECHA_MOV').AsDateTime;
                 cdsMovsORIGEN_MOVIMIENTO.Value := FieldByName('ORIGEN_MOVIMIENTO').AsInteger;
                 cdsMovsID_TIPO_CAPTACION.Value := FieldByName('ID_TIPO_CAPTACION').AsInteger;
                 cdsMovsNUMERO_CUENTA.Value := FieldByName('NUMERO_CUENTA').AsInteger;
                 cdsMovsNOMBRE.Value := FieldByName('PRIMER_APELLIDO1').AsString + ' ' +FieldByName('SEGUNDO_APELLIDO1').AsString + ' ' + FieldByName('NOMBRE1').AsString;
                 cdsMovsDOCUMENTO.Value := FieldByName('DOCUMENTO').AsString;
                 cdsMovsID_TIPO_MOVIMIENTO.Value := FieldByName('ID_TIPO_MOVIMIENTO').AsInteger;
                 if FieldByName('ID_TIPO_MOVIMIENTO').AsInteger = 1 then
                 begin
                      cdsMovsEFECTIVO_DB.Value := FieldByName('BILLETES').AsCurrency + FieldByName('MONEDAS').AsCurrency;
                      cdsMovsCHEQUE_DB.Value := FieldByName('CHEQUES').AsCurrency;
                      cdsMovsEFECTIVO_CR.Value := 0;
                      cdsMovsCHEQUE_CR.Value := 0;
                 end
                 else
                 begin
                      cdsMovsEFECTIVO_CR.Value := FieldByName('BILLETES').AsCurrency + FieldByName('MONEDAS').AsCurrency;
                      cdsMovsCHEQUE_CR.Value := FieldByName('CHEQUES').AsCurrency;
                      cdsMovsEFECTIVO_DB.Value := 0;
                      cdsMovsCHEQUE_DB.Value := 0;
                 end;
                 cdsMovs.Post;
...
// Registrar Entradas
                       ...
                       begin
                          ...
                          IBSQLAux.ParamByName('TIPO_COMPROBANTE').AsInteger := 1;
                          try
                           IBSQLAux.ExecQuery;
                           NoHay := False;
                          except
...
// Registrar Otros Descuentos
        ...
        with IBSQL1 do begin
          ...
          SQL.Add('WHERE ID_AGENCIA = :AGENCIA and ID_COMPROBANTE = :COMPROBANTE and TIPO_COMPROBANTE = :TIPO_COMPROBANTE');
          ...
          SQL.Add('IMPRESO = 1 WHERE ID_AGENCIA = :ID_AGENCIA and ID_COMPROBANTE = :ID_COMPROBANTE and TIPO_COMPROBANTE = :TIPO_COMPROBANTE');
          ParamByName('ID_AGENCIA').AsInteger := Agencia;
          ParamByName('ID_COMPROBANTE').AsInteger := Consecutivo;
          ParamByName('TIPO_COMPROBANTE').AsInteger := 1;          
...
procedure TfrmComprobanteDiario.NotasRecaudo;
var
 _caja : Currency;
 _valor : Currency;
 Fecha1, Fecha2 : TDateTime;
 NoHay : Boolean;
 _tipoCaptacion: Integer;
 _numeroCuenta: Integer;
 _digitoControl: Integer;
begin
        Fecha1 := EdFecha.Date;
        Fecha1 := EncodeDate(YearOf(Fecha1),MonthOf(Fecha1),DayOf(Fecha1));
        Fecha2 := EdFecha.Date;
        ...
        begin
           ...
           Open;
             ...
             while not eof do
             begin
                  Consecutivo := ObtenerConsecutivo(IBSQLconsec);
                  _lista.Add(IntToStr(Consecutivo));

                  IBQuery1.Close;
                  IBQuery1.SQL.Clear;
                  IBQuery1.SQL.Add('INSERT INTO CON$COMPROBANTE VALUES(');
                  IBQuery1.SQL.Add(':ID_COMPROBANTE,:ID_AGENCIA,:TIPO_COMPROBANTE,:FECHADIA,');
                  IBQuery1.SQL.Add(':DESCRIPCION,:TOTAL_DEBITO,:TOTAL_CREDITO,:ESTADO,:IMPRESO,');
                  IBQuery1.SQL.Add(':ANULACION,:ID_EMPLEADO)');
                  IBQuery1.ParamByName('ID_COMPROBANTE').AsInteger := Consecutivo;
                  IBQuery1.ParamByName('ID_AGENCIA').AsInteger := Agencia;
                  IBQuery1.ParamByName('TIPO_COMPROBANTE').AsInteger := 1;
                  IBQuery1.ParamByName('FECHADIA').AsDate := EdFecha.Date;
                  IBQuery1.ParamByName('DESCRIPCION').AsString := 'Contabilización de Recaudo de '+ IBQRecaudo.FieldByName('DESCRIPCION_RECAUDO').AsString + ', Caja: ' +
                                                           EdCaja.Caption + ' '+ EdUsuario.Caption +
                                                           ' en la Fecha';
                  IBQuery1.ParamByName('TOTAL_DEBITO').AsCurrency := IBQRecaudo.FieldByName('EFECTIVO').AsCurrency;
                  IBQuery1.ParamByName('TOTAL_CREDITO').AsCurrency := IBQRecaudo.FieldByName('EFECTIVO').AsCurrency;
                  IBQuery1.ParamByName('ESTADO').AsString := 'O';
                  IBQuery1.ParamByName('IMPRESO').AsInteger := 0;
                  IBQuery1.ParamByName('ANULACION').Clear;
                  IBQuery1.ParamByName('ID_EMPLEADO').AsString := DBAlias;
                  try
                    IBQuery1.ExecSQL;
                  except
                    MessageDlg('Error al Grabar Encabezado de Comprobante Recaudo ' + IBQRecaudo.FieldByName('DESCRIPCION_RECAUDO').AsString,mtError,[mbcancel],0);
                    Transaction.Rollback;
                    Exit;
                  end;
                  ...
                  IBSQLAux.ParamByName('ID_AGENCIA').AsInteger := Agencia;
                  IBSQLAux.ParamByName('FECHA').AsDate := EdFecha.Date;
                  IBSQLAux.ParamByName('CODIGO').AsString := IBQRecaudo.FieldByName('CODIGO_RECAUDO').AsString;
                  IBSQLAux.ParamByName('DEBITO').AsCurrency := 0;
                  IBSQLAux.ParamByName('CREDITO').AsCurrency := IBQRecaudo.FieldByName('EFECTIVO').AsCurrency;
                  IBSQLAux.ParamByName('ID_CUENTA').Clear;
                  IBSQLAux.ParamByName('ID_COLOCACION').Clear;
                  IBSQLAux.ParamByName('ID_IDENTIFICACION').AsInteger := 0;
                  IBSQLAux.ParamByName('ID_PERSONA').Clear;
                  ...
                  IBSQLAux.ParamByName('FECHA').AsDate := EdFecha.Date;
                  IBSQLAux.ParamByName('CODIGO').AsString := CodigoCaja;
                  IBSQLAux.ParamByName('DEBITO').AsCurrency := IBQRecaudo.FieldByName('EFECTIVO').AsCurrency;
                  IBSQLAux.ParamByName('CREDITO').AsCurrency := 0;
                  IBSQLAux.ParamByName('ID_CUENTA').Clear;
                  IBSQLAux.ParamByName('ID_COLOCACION').Clear;
                  IBSQLAux.ParamByName('ID_IDENTIFICACION').AsInteger := 0;
                  ...
                  _tipoCaptacion := FieldByName('ID_TIPO_CAPTACION').AsInteger;
                  _numeroCuenta := FieldByName('NUMERO_CUENTA').AsInteger;
                  _digitoControl := StrToInt(DigitoControl(_tipoCaptacion, IntToStr(_numeroCuenta)));


                  IBQuery1.Close;
                  IBQuery1.SQL.Clear;
                  IBQuery1.SQL.Add('insert into "cap$extracto" values(');
                  IBQuery1.SQL.Add(':"ID_AGENCIA",:"ID_TIPO_CAPTACION",:"NUMERO_CUENTA",');
                  IBQuery1.SQL.Add(':"DIGITO_CUENTA",:"FECHA_MOVIMIENTO",:"HORA_MOVIMIENTO",');
                  IBQuery1.SQL.Add(':"ID_TIPO_MOVIMIENTO",:"DOCUMENTO_MOVIMIENTO",:"DESCRIPCION_MOVIMIENTO",');
                  IBQuery1.SQL.Add(':"VALOR_DEBITO",:"VALOR_CREDITO", :"ID")');
                  IBQuery1.ParamByName('ID_AGENCIA').AsInteger := Agencia;
                  IBQuery1.ParamByName('ID_TIPO_CAPTACION').AsInteger := _tipoCaptacion;
                  IBQuery1.ParamByName('NUMERO_CUENTA').AsInteger := _numeroCuenta;
                  IBQuery1.ParamByName('DIGITO_CUENTA').AsInteger := _digitoControl;
                  IBQuery1.ParamByName('FECHA_MOVIMIENTO').AsDateTime := EdFecha.Date;
                  IBQuery1.ParamByName('HORA_MOVIMIENTO').AsTime := fHoraActual;
                  IBQuery1.ParamByName('ID_TIPO_MOVIMIENTO').AsInteger := 1;
                  IBQuery1.ParamByName('DOCUMENTO_MOVIMIENTO').AsString := Format('%.7d',[Consecutivo]);
                  IBQuery1.ParamByName('DESCRIPCION_MOVIMIENTO').AsString := 'CONSIGNACION EFECTIVO RECAUDOS EN LA FECHA';
                  IBQuery1.ParamByName('VALOR_DEBITO').AsCurrency := IBQRecaudo.FieldByName('EFECTIVO').AsCurrency;
                  IBQuery1.ParamByName('VALOR_CREDITO').AsCurrency := 0;
                  IBQuery1.ParamByName('ID').AsInteger := 0;
                  IBQuery1.ExecSQL;
          ...
          EdComprobante.Text := _lista.GetText;
end;
...
begin
        ...
        with IBAuxiliar do begin
         ...
         SQL.Add('           CON$COMPROBANTE.ID_COMPROBANTE IN ('+ _lista.CommaText + ') )');
         SQL.Add('Order By CON$COMPROBANTE.ID_COMPROBANTE,CON$AUXILIAR.CREDITO ASC, CON$AUXILIAR.CODIGO ASC');
         ParamByName('ID_AGENCIA').AsInteger := Agencia;
         try
          Open;
          {
