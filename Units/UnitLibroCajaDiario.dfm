object frmLibroRCajaDiario: TfrmLibroRCajaDiario
  ...
  object IBQPuc: TIBQuery
    ...
    SQL.Strings = (
        ...
        'SELECT EXTRACT(DAY FROM a.FECHA) AS DIA, SUBSTRING(a.CODIGO FROM' +
        ' 1 FOR 4) AS CODIGO, p.NOMBRE, SUM(a.DEBITO) AS DEBITO, SUM(a.CR' +
        'EDITO) AS CREDITO'
      'FROM CON$AUXILIAR a'
  ...
  object Reporte: TprTxReport
    ...
    object gCodigo: TprGroup
      Valid = 'IBQTemp1.CODIGO'
      DetailBand = Reporte.prTxHDetailBand2
    end
  end
  object IBQAuxiliar: TIBQuery
    ...
    SQL.Strings = (
        ...
        'select CON$CAJADIARIO.CODIGO, CON$PUC.NOMBRE, SUM("con$cajad' +
        'iario".DEBITO)  AS DEBITO, SUM(CON$CAJADIARIO.CREDITO)  AS CRE' +
        'DITO'
      'from CON$CAJADIARIO'
      
        'left join CON$PUC ON (CON$CAJADIARIO.CODIGO = CON$PUC.CODI' +
        'GO)'
      'where'
      'CON$CAJADIARIO.MES = :MES'
      'group by'
      'CON$CAJADIARIO.CODIGO,CON$PUC.NOMBRE'
      'order by  CON$CAJADIARIO.CODIGO ASC')
    Left = 34
    Top = 24
    ParamData = <
      item
  ...
  object IBQTemp1: TClientDataSet
    Active = True
    Aggregates = <
      item
        Active = True
        AggregateName = 'TOTALD'
    ...
    Data = {
      ...
      49474F01004900000001000557494454480200020012000644454249544F0800
      04000000010007535542545950450200490006004D6F6E657900074352454449
      544F080004000000010007535542545950450200490006004D6F6E657900064E
      4F4D42524501004900000001000557494454480200020064000000}
    ...
    object TOTALC: TAggregateField
      FieldName = 'TOTALC'
      Visible = True
      Active = True
      DisplayName = 'TCREDITO'
      Expression = 'SUM(CREDITO)'
      GroupingLevel = 1
      IndexName = 'CODIGOIDX'
    end
