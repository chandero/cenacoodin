unit UnitAsignarTarjetaDebitoN;
...
type
  TfrmAsignarTarjetaDebitoN = class(TForm)
    ...
    IBAgencias: TIBQuery;
    frMarcarPTD: TfrReport;
    MarcarPlstico1: TMenuItem;
    Formato11: TMenuItem;
    Formato21: TMenuItem;
    prMarcarPTD: TprReport;
    Panel1: TPanel;
    Label1: TLabel;
    ...
    GroupBox5: TGroupBox;
    DBGCanal: TDBGrid;
    CDScanal: TClientDataSet;
    CDScanalVICA_ID: TIntegerField;
    CDScanalVICA_DESCRIPCION: TStringField;
    CDScanalVICA_NOPE: TIntegerField;
    CDScanalVICA_MONTO: TCurrencyField;
    DSCanal: TDataSource;
    IBQvirtual_canal: TIBQuery;
    ...
    procedure Formato11Click(Sender: TObject);
    procedure Formato21Click(Sender: TObject);
  private
...
var
  frmAsignarTarjetaDebitoN: TfrmAsignarTarjetaDebitoN;
  MyListAho: TList;
  Tarjeta:string;
  Existencia:Integer;
...
procedure TfrmAsignarTarjetaDebitoN.CmdCerrarClick(Sender: TObject);
begin
        Close;
end;

procedure TfrmAsignarTarjetaDebitoN.FormShow(Sender: TObject);
begin
        Inicializar;
end;

procedure TfrmAsignarTarjetaDebitoN.EdNumeroAhoExit(Sender: TObject);
...
begin
        ...
        begin
            ...
            Close;
            SQL.Clear;
            SQL.Add('select * from VITUAL_TARJETA_CUENTA t INNER JOIN VIRTUAL_CUENTA c ON t.VICU_ID = c.VICU_ID WHERE');
            SQL.Add('VICU_TIPO_CUENTA = :ID_TIPO_CAPTACION and');
            SQL.Add('VICU_NUMERO_CUENTA = :NUMERO_CUENTA and VICU_DIGITO_CUENTA = :DIGITO_CUENTA and');
            SQL.Add('((c.VITC_ESTADO = 1) or (c.VITC_ESTADO = 2) or (c.VITC_ESTADO = 3))');
            ParamByName('ID_TIPO_CAPTACION').AsInteger := DBLCBTiposCaptacion.KeyValue;
            ParamByName('NUMERO_CUENTA').AsInteger := StrToInt(EdNumeroAho.Text);
            ParamByName('DIGITO_CUENTA').AsInteger := StrToInt(EdDigitoAho.Caption);
            try
...
//                 CmdAsignar.Enabled := False;
                 ...
                 if FieldByName('VITC_ESTADO').AsInteger = 1 then
                 begin
                 ...
                 if FieldByName('VITC_ESTADO').AsInteger = 2 then
                 begin
                 ...
                 if FieldByName('VITC_ESTADO').AsInteger = 3 then
                 begin
...
//                  CmdAsignar.SetFocus;
        ...
        IBQHistoria.SQL.Add('select * from VITUAL_TARJETA_CUENTA t INNER JOIN VIRTUAL_CUENTA c ON t.VICU_ID = c.VICU_ID WHERE ');
        IBQHistoria.SQL.Add('  VICU_TIPO_CUENTA = :ID_TIPO_CAPTACION and');
        IBQHistoria.SQL.Add('  VICU_NUMERO_CUENTA = :NUMERO_CUENTA and VICU_DIGITO_CUENTA = :DIGITO_CUENTA and');
        IBQHistoria.SQL.Add('  c.VITC_ESTADO = 3');
        ...
        with IBQvirtual_canal do
        begin
            Open;
            CDScanal.Open;
            CDScanal.EmptyDataSet;
            while not Eof do
            begin
                 CDScanal.Insert;
                 CDScanalVICA_ID.Value := FieldByName('VICA_ID').AsInteger;
                 CDScanalVICA_DESCRIPCION.Value := FieldByName('VICA_DESCRIPCION').AsString;
                 CDScanalVICA_NOPE.Value := FieldByName('VICA_NOPE').AsInteger;
                 CDScanalVICA_MONTO.Value := FieldByName('VICA_MONTO').AsCurrency;
                 CDScanal.Post;
                 Next;
            end;
        end;
...
procedure TfrmAsignarTarjetaDebitoN.EdNumeroAhoKeyPress(Sender: TObject;
  var Key: Char);
begin
        NumericoSinPunto(Sender,Key);
end;

procedure TfrmAsignarTarjetaDebitoN.ActualizarListAho;
var i:Integer;
    ARecord:PMyListAho;
begin
...
procedure TfrmAsignarTarjetaDebitoN.GridTitularesClick(Sender: TObject);
var ARecord:PMyListAho;
begin
...
procedure TfrmAsignarTarjetaDebitoN.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self);
end;

procedure TfrmAsignarTarjetaDebitoN.Inicializar;
var
  Limite:Integer;
begin
...
procedure TfrmAsignarTarjetaDebitoN.CmdOtraClick(Sender: TObject);
begin
        Inicializar;
end;

procedure TfrmAsignarTarjetaDebitoN.CmdContinuarClick(Sender: TObject);
begin
...
procedure TfrmAsignarTarjetaDebitoN.CmdAsignarClick(Sender: TObject);
        ...
        while True do
         with IBMarcarTarjeta do begin
           ...
           try
             ...
             Close;
             SQL.Clear;
             SQL.Add('select FIRST 1 * from TARJETADEBITO where TADE_ASIGNADA = 0 ORDER BY TADE_TARJETA');
             ExecQuery;
             Tarjeta := FieldByName('TADE_TARJETA').AsString;
             Close;
             SQL.Clear;
             SQL.Add('UPDATE TARJETADEBITO SET ASIGNADA = 1, TADE_ESTADO = 1 WHERE TADE_TARJETA = TADE_TARJETA');
             ParamByName('TADE_TARJETA').AsString := Tarjeta;
             ExecQuery;
             Transaction.Commit;
             Break;
           except
...
procedure TfrmAsignarTarjetaDebitoN.CmdBloquearClick(Sender: TObject);
begin
        ...
        with IBSQL1 do begin
{
          ...
          try
            ...
            if EstadoActual = 2 then
              sentencia := 'update TDB$TARJETA set ID_ESTADO = 1 where ID_TARJETA = ' + QuotedStr(Tarjeta)
            else
              sentencia := 'update TDB$TARJETA set ID_ESTADO = 2 where ID_TARJETA = ' + QuotedStr(Tarjeta);
            ...
            XmlRes.SaveToFile('c:\TBloquear' + IntToStr(Agencia) + '.xml');

            ShowMessage('Registro en la Base Consolidada Exitoso!');
          except
            on e: Exception do
            begin
          ...
          //Fin Marcar Tarjeta en Consolidada
...
procedure TfrmAsignarTarjetaDebitoN.CmdCancelarClick(Sender: TObject);
begin
        if not ValidarOpcion((Sender as TButton).Tag) then
        begin
           Exit;
        ...
        with IBSQL1 do begin
          ...
          try
           ...
           try
             ...
             XmlPet.XmlFormat := xfReadable;
             Nodo := XmlPet.Root.NodeNew('querys');
             sentencia := 'update TDB$TARJETA set ID_ESTADO = 3 where ID_TARJETA = ' + QuotedStr(Tarjeta);
             Nodo1 := Nodo.NodeNew('query');
             Nodo1.WriteString('tipo','update');
             Nodo1.WriteString('sentencia',sentencia);
             XmlPet.SaveToFile('c:\XmlPet.xml');
             ...
             XmlPet.SaveToStream(Astream);
             _TXml.AstreamEnt := EnviarXml(IBAgencias.FieldByName('ID_PUERTO').AsInteger,IBAgencias.FieldByName('ID_HOST').AsString,'',AStream);
             XmlRes.SaveToFile('c:\TAsignar' + IntToStr(Agencia) + '.xml');
...
procedure TfrmAsignarTarjetaDebitoN.CmdModificarClick(Sender: TObject);
begin
        if not ValidarOpcion((Sender as TButton).Tag) then
        begin
           Exit;
...
procedure TfrmAsignarTarjetaDebitoN.CicloSaldos;
var
  Dia1:TDate;
  Dia2:TDate;
...
procedure TfrmAsignarTarjetaDebitoN.DBRangosDrawColumnCell(Sender: TObject;
...
function TfrmAsignarTarjetaDebitoN.ValidarAsociado: Boolean;
var
  Ag:Integer;
  Tp:Integer;
  Nm:Integer;
...
procedure TfrmAsignarTarjetaDebitoN.FormCreate(Sender: TObject);
begin
...
procedure TfrmAsignarTarjetaDebitoN.FormCloseQuery(Sender: TObject;
...
procedure TfrmAsignarTarjetaDebitoN.Mnu01Click(Sender: TObject);
var
  Ano,Mes,Dia:Integer;
  Nombre:string;
  Documento:string;
...
procedure TfrmAsignarTarjetaDebitoN.Mnu02Click(Sender: TObject);
begin
// Imprimir Comprobante
        with IBAuxiliar do
        begin
...
procedure TfrmAsignarTarjetaDebitoN.IBQRangosAfterScroll(DataSet: TDataSet);
begin
        if Iniscroll then
          Iniscroll := False
        else
...
function TfrmAsignarTarjetaDebitoN.EnviarXml(iIdPuerto:Integer;sIdHost,Desc:string;AstreamEnv:TMemoryStream): TMemoryStream;
var     Cadena :string;
        AStream : TMemoryStream;
        Tamano :Integer;
begin
...
procedure TfrmAsignarTarjetaDebitoN.Formato11Click(Sender: TObject);
var
  _sNombre: string;
  _sSegundoN: string;
  _sPrimerA: string;
...
begin
           ...
           begin
               ...
               if RecordCount = 1 then
               begin
                _sPrimerA := FieldByName('PRIMER_APELLIDO').AsString;
                _sSegundoA := FieldByName('SEGUNDO_APELLIDO').AsString;
                _sNombre := FieldByName('NOMBRE').AsString;
               end;
               Close;
           end;

        _sNombre := Trim(_sNombre);
        _iFpos := Pos(' ',_sNombre);

        if _iFpos = 0 then
        begin
           _iFpos := Length(_sNombre);
           _sSegundoN := '';
        end
        else
        begin
          _sSegundoN := MidStr(_sNombre,_iFpos+1,1);
          _sSegundoN := _sSegundoN + '.';
        end;



        _sNombre := LeftStr(_sNombre,_iFpos);
        _sNombre := LowerCase(_sNombre);
        _sCapital := UpperCase(LeftStr(_sNombre,1));
        _sNombre := _sCapital + RightStr(_sNombre,Length(_sNombre) - 1);

        _sSegundoA := Trim(_sSegundoA);
        if ( (_sSegundoA <> '') and (_sSegundoN <> ' ') ) then
           _sSegundoA := LeftStr(_sSegundoA,1) + '.'
        else
          _sSegundoA := '';

        _sPrimerA := Trim(_sPrimerA);
        _sPrimerA := LowerCase(_sPrimerA);
        _sCapital := UpperCase(LeftStr(_sPrimerA,1));
        _sPrimerA := _sCapital + RightStr(_sPrimerA,Length(_sPrimerA) - 1);

        _sNombre := Trim(_sNombre);

        if _sSegundoN <> '' then
          _sNombre := _sNombre + ' ' + _sSegundoN;
        if _sPrimerA <> '' then
          _sNombre := _sNombre + ' ' + _sPrimerA;
        if _sSegundoA <> '' then
          _sNombre := _sNombre + ' ' + _sSegundoA;

        frMarcarPTD.LoadFromFile(ExtractFilePath(Application.ExeName)+'\ReportesCap\MarcarPlasticoTD.frf');
        frMarcarPTD.Dictionary.Variables.Variable['NOMBRE'] := QuotedStr(_sNombre);
        frMarcarPTD.ShowReport;

end;

procedure TfrmAsignarTarjetaDebitoN.Formato21Click(Sender: TObject);
var
  _sNombre: string;
  _sSegundoN: string;
  _sPrimerA: string;
...
begin
           ...
           begin
               ...
               Close;
               if DBLCBTiposCaptacion.KeyValue = 2 then
               begin
                 SQL.Clear;
                 SQL.Add('select COUNT(0) AS CONTADOR from "cap$maestro" where NUMERO_CUENTA = :NUMERO_CUENTA AND ID_TIPO_CAPTACION = 1');
                 ParamByName('NUMERO_CUENTA').AsInteger := StrToInt(EdNumeroAho.Text);
                 ExecQuery;
        ...
        if _sSegundoA <> '' then
          _sNombre := _sNombre + ' ' + _sSegundoA;
          _sNombre := _sNombre + ' ' + _sTipo + FormatCurr('00',Agencia);


        prMarcarPTD.LoadTemplateFromFile(ExtractFilePath(Application.ExeName)+'ReportesCap\MarcarPlasticoTD.prt', False);
        prMarcarPTD.Variables.ByName['NOMBRE'].AsString := _sNombre;
        if prMarcarPTD.PrepareReport then
          prMarcarPTD.PreviewPreparedReport(True);
