...
type
  TFrmSolicitudCre = class(TForm)
    ...
    CDrequisitosdescripcion: TStringField;
...
begin
      ...
      _sLog := ValidaIncapacitado(DBTipo.KeyValue,JvDocumento.Text);
...
begin
        ...
        begin
           if codeudores.Value >= 2 then
              ParValue := '(ES) ' +  NUMEROS[codeudores.Value]+'('+IntToStr(codeudores.Value)+')'
           else
        ...
        if ParName = 'textociudad' then
           ParValue := ParametroRep(3);
        if ParName = 'textoemail' then
           ParValue := ParametroRep(2);
...
begin
        ...
        begin
          Close;
          SQL.Clear;
          SQL.Add('select VALOR_ACTUAL_TASA from "col$tasasvariables" WHERE ID_INTERES = 1');
          Open;
          _cTasa := FieldByName('VALOR_ACTUAL_TASA').AsCurrency;
          Close;               
          SQL.Clear;
          SQL.Add('SELECT');
          SQL.Add('"col$lineas".VECES_APORTES,TASA,CAPITALIZACION_ACUMULADA,PUNTOS_ADICIONALES');
          SQL.Add('FROM');
          SQL.Add('"col$lineas"');
          SQL.Add('WHERE');
          ...
          _iPuntos := FieldByName('PUNTOS_ADICIONALES').AsCurrency;
          if _iPuntos > 0 then
          begin
            JVinteres.Value := _cTasa + _iPuntos;
          end
          else
