...
uses
  ...
  IdComponent, IdTCPConnection, IdTCPClient,Math,Jpeg;
...
type
  TfrmConsultaProductos = class(TForm)
    ...
    IBTPersona: TIBTransaction;
    Label103: TLabel;
    EdIncentivo: TJvStaticText;
    EdEstadoContractual: TJvStaticText;
    EdPlanContractual: TJvStaticText;
    GroupBox7: TGroupBox;
    EdCuentaTarjeta: TStaticText;
    ImgFotoC: TImage;
    ImFirma: TImage;
             ...
             while not Eof do begin
               ...
               if (obtenerdeudahoy1(Agencia,FieldByName('ID_COLOCACION').AsString,IBDeudaHoy).Dias > 0)  then
                if (FieldByName('ID_ESTADO_COLOCACION').AsInteger = 0) then begin
                  ...
                  IBSQL2.SQL.Clear;
                  IBSQL2.SQL.Add('select');
                  IBSQL2.SQL.Add('sum("col$tablaliquidacion".CAPITAL_A_PAGAR) AS TOTAL');
                  ...
                  begin
                     Vcapital :=IBSQL2.FieldByName('TOTAL').AsCurrency;
                     IBSQL2.Next;
                  end;
...
begin
       ...
       begin
             ...
             SQL.Add('("col$solicitud".ESTADO IN (1,2,3,4,8,11,12,13,14,7,9,18))');
             ParamByName('ID_PERSONA').AsString := vidpersona;
             ParamByName('ID_IDENTIFICACION').AsInteger := vididentificacion;
             Open;
...
begin
        ...
        begin
          Close;
          SQL.Clear;
          SQL.Add('SELECT SALIDA FROM P_COL_CREDIVIDA(:IDPERSONA)');
          ParamByName('IDPERSONA').AsString := vidpersona;
          Open;
          Last;
          First;
          if RecordCount > 0 then
          begin
            ...
            while not Eof do
            begin
              NodoSe[RecNo+2] := Arbol.Items.AddChildObject(NodoSe[0],FieldByName('SALIDA').AsString,arbolinfo);
              NodoSe[RecNo+2].ImageIndex := 5;
              Next;
            end;//fin while
        ...
        begin
           ...
           try
            ...
            begin
              ...
              EdCuentaTarjeta.Caption := FieldByName('ID_TIPO_CAPTACION').AsString + '0' + IntToStr(Agencia) + '-' + FormatCurr('000000',FieldByName('NUMERO_CUENTA').AsInteger);
            end;
           except
            Transaction.Rollback;
            raise;
...
begin
        ...
        begin
          ...
          SQL.Add('INNER JOIN "col$estadosolicitud" ON ("col$solicitud".ESTADO="col$estadosolicitud".ID_ESTADO)');
          SQL.Add('WHERE');
          SQL.Add('("col$solicitud".ESTADO IN (1,2,3,4,7,8,9,10,13,14)) AND');
