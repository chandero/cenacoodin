unit UnitInformeCajaAuditoria;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, IBSQL, DBClient, DB, IBCustomDataSet, IBQuery, pr_Common, pr_Parser,
  pr_TxClasses, StdCtrls, Buttons, JvEdit, JvTypedEdit, ComCtrls, ExtCtrls,
  DateUtils;

type
  TfrmInformeCajaAuditoria = class(TForm)
    Panel1: TPanel;
    Label1: TLabel;
    Caja: TLabel;
...
var
  frmInformeCajaAuditoria: TfrmInformeCajaAuditoria;
  Generado:Boolean;
  Tabla:string;
...
procedure TfrmInformeCajaAuditoria.ReportUnknownObjFunction(Sender: TObject;
  Component: TComponent; const FuncName: String;
  const Parameters: TprVarsArray; ParametersCount: Integer;
  var Value: TprVarValue; var IsProcessed: Boolean);
var f:TField;
...
procedure TfrmInformeCajaAuditoria.FormShow(Sender: TObject);
begin
...
procedure TfrmInformeCajaAuditoria.CmdGeneraClick(Sender: TObject);
var Fecha1,Fecha2:TDateTime;
    frmVistaPreliminar :TfrmVistaPreliminar;
    Cajero:string;
begin
       Empleado;
       Tabla := '"diariocajaud' + IntToStr(EdCaja.Value)+FloatToStr(Time)+'"';
        ...
        begin
           ...
           SQL.Add('FROM');
           SQL.Add(' CAJ$CAJAS');
           SQL.Add(' INNER JOIN "gen$empleado" ON');
           SQL.Add(' (CAJ$CAJAS.ID_EMPLEADO="gen$empleado".ID_EMPLEADO)');
           SQL.Add('WHERE');
           SQL.Add('CAJ$CAJAS.ID_CAJA = :ID_CAJA');
           ParamByName('ID_CAJA').AsInteger := EdCaja.Value;
           try
            ExecQuery;
            Cajero := Trim(FieldByName('PRIMER_APELLIDO').AsString) + ' ' +
...
//           IBQuery1.SQL.Add('order by ID_AGENCIA,ORIGEN_MOVIMIENTO,ID_TIPO_MOVIMIENTO,FECHA_MOV');
           ...
           IBQuery3.SQL.Add('where FECHA_MOV between :FECHA1 and :FECHA2');
           if not chkTodas.Checked then
              IBQuery3.SQL.Add('AND ID_CAJA = :ID_CAJA');
            case  rgorden.ItemIndex of
            0: begin
             ...
             try
               ...
               if (IBQuery1.RecordCount > 0) or (IBQuery3.RecordCount > 0) then
                begin
                  ...
                  Report.Variables.ByName['Cajero'].AsString := Cajero;
...
procedure TfrmInformeCajaAuditoria.FormKeyPress(Sender: TObject; var Key: Char);
begin
        EnterTabs(Key,Self);
end;

procedure TfrmInformeCajaAuditoria.FormCloseQuery(Sender: TObject;
...
procedure TfrmInformeCajaAuditoria.CmdCerrarClick(Sender: TObject);
begin
        Close;
end;
