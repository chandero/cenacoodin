         ...
         begin
            ...
            if captacion then GrabarCaptacion(Arecord^.nocuenta,Arecord^.debito,Arecord^.credito,Arecord^.esretirocheque);
             if ARecord^.credito > 0 then
               if ARecord^.gmf then
                 if (ARecord^.codigo = '110505000000000000'{YA ES NIIF}) then
                    TPAGOSCAJA := TPAGOSCAJA + ARecord^.Credito
                 else
                 if (ARecord^.codigo = '111005010000000000'{YA ES NIIF}) or
                    (ARecord^.codigo = '111005030000000000'{YA ES NIIF}) or
                    (ARecord^.codigo = '111005050000000000'{YA ES NIIF}) or
                    (ARecord^.codigo = '111005070000000000'{YA ES NIIF}) or
                    (ARecord^.codigo = '111010010000000000'{YA ES NIIF}) or Arecord^.banco then begin
                    TPAGOS := TPAGOS + ARecord^.Credito;
                    CodigoPago := ARecord^.Codigo;
                 end;
             if ARecord^.debito > 0 then
               if ARecord^.gmf then
                 if (ARecord^.codigo = '210505010000000000'{YA ES NIIF}) or
                    (ARecord^.codigo = '210505030000000000'{YA ES NIIF}) then
                     TAHOV := TAHOV + Arecord^.debito
                 else
                 if (ARecord^.codigo = '212505000000000000'{YA ES NIIF}) or
                    (ARecord^.codigo = '212515000000000000'{YA ES NIIF}) then
                     TAHOC := TAHOC + ARECORD^.debito
                 else
                 if (ARecord^.codigo = '310505000000000000'{YA ES NIIF}) then
                    TAPO := TAPO + arecord^.debito
                 else
                 if (ARecord^.codigo = '212595000000000000'{YA ES NIIF}) then
            ...
            if TAHOV > 0 then begin
               ...
               ParamByName('ID_AGENCIA').AsInteger:= Agencia;
               ParamByName('FECHA').AsDate := fFechaActual;
               ParamByName('CODIGO').AsString := '243005000000000000'{YA ES NIIF};
            ...
            if TAHOC > 0 then begin
               ...
               ParamByName('ID_AGENCIA').AsInteger:= Agencia;
               ParamByName('FECHA').AsDate := fFechaActual;
               ParamByName('CODIGO').AsString := '243015000000000000'{YA ES NIIF};
               ParamByName('DEBITO').AsCurrency := 0.00;
            ...
            if (TAPO+TPAGOSCAJA+TPAGOSINT) > 0 then begin
               ...
               ParamByName('ID_AGENCIA').AsInteger:= Agencia;
               ParamByName('FECHA').AsDate := fFechaActual;
               ParamByName('CODIGO').AsString := '243095000000000000'{YA ES NIIF};
               ParamByName('DEBITO').AsCurrency := 0.00;
            ...
            if (TAHOV) > 0 then begin
               ...
               ParamByName('ID_AGENCIA').AsInteger:= Agencia;
               ParamByName('FECHA').AsDate := fFechaActual;
               ParamByName('CODIGO').AsString := '523050000000000000'{YA ES NIIF};
            ...
            if (TAHOC+TAPO+TPAGOSINT+TPAGOSCAJA+TPAGOS) > 0 then begin
               ...
               ParamByName('ID_AGENCIA').AsInteger:= Agencia;
               ParamByName('FECHA').AsDate := fFechaActual;
               ParamByName('CODIGO').AsString := '615025000000000000'{YA ES NIIF};
       ...
       try
         ...
         begin
            ...
            ExecSQL;
             if ARecord^.credito > 0 then
               if ARecord^.gmf then
                 if (ARecord^.codigo = '110505000000000000'{YA ES NIIF}) or
                    (ARecord^.codigo = '111005010000000000'{YA ES NIIF}) or
                    (ARecord^.codigo = '111005030000000000'{YA ES NIIF}) or
                    (ARecord^.codigo = '111005050000000000'{YA ES NIIF}) or
                    (ARecord^.codigo = '111005070000000000'{YA ES NIIF}) or
                    (ARecord^.codigo = '111010010000000000'{YA ES NIIF}) or Arecord^.banco then
                    TPAGOS := TPAGOS + ARecord^.credito;
             if ARecord^.debito > 0 then
               if ARecord^.gmf then
                 if (ARecord^.codigo = '210505010000000000'{YA ES NIIF}) or
         ...
         SQL.Add(':"ID_COMPROBANTE",:"ID_AGENCIA",:"FECHA",:"CODIGO",:"DEBITO",');
         SQL.Add(':"CREDITO",:"ID_CUENTA",:"ID_COLOCACION",:"ID_IDENTIFICACION",');
         SQL.Add(':"ID_PERSONA",:"MONTO_RETENCION",:"TASA_RETENCION",:"ESTADOAUX", :"TIPO-COMPROBANTE")');
   ...
   with list do
        ...
        if ARecord^.credito > 0 then
          if ARecord^.gmf then
            ...
            if (ARecord^.codigo = '111005010000000000'{YA ES NIIF}) or
               ...
               (ARecord^.codigo = '111005050000000000'{YA ES NIIF}) or
               (ARecord^.codigo = '111005070000000000'{YA ES NIIF}) or
               (ARecord^.codigo = '111005090000000000'{YA ES NIIF}) or ARecord^.banco then begin
               TPAGOS := TPAGOS + ARecord^.credito;
               CodigoPago := ARecord^.codigo;
     ...
     if not vmodificar then begin
       if TAHOV > 0 then begin
          ...
          xsauxiliar.Cells [0,XSauxiliar.RowCount-1] := '243005000000000000'{YA ES NIIF};
       ...
       if TAHOC > 0 then begin
          XSauxiliar.RowCount := XSauxiliar.RowCount+1;
          xsauxiliar.Cells [0,XSauxiliar.RowCount-1] := '243015000000000000'{YA ES NIIF};
       ...
       if (TAPO+TPAGOSCAJA+TPAGOSINT) > 0 then begin
          XSauxiliar.RowCount := XSauxiliar.RowCount+1;
          xsauxiliar.Cells [0,XSauxiliar.RowCount-1] := '243095000000000000'{YA ES NIIF};
       ...
       if (TAHOV) > 0 then begin
          XSauxiliar.RowCount := XSauxiliar.RowCount+1;
          xsauxiliar.Cells [0,XSauxiliar.RowCount-1] := '523050000000000000'{YA ES NIIF};
       ...
       if (TAHOC+TAPO+TPAGOSINT+TPAGOSCAJA+TPAGOS) > 0 then begin
          XSauxiliar.RowCount := XSauxiliar.RowCount+1;
          xsauxiliar.Cells [0,XSauxiliar.RowCount-1] := '615025000000000000'{YA ES NIIF};
