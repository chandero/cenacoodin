...
uses
  ...
  ImgList, FR_DSet, FR_DBSet, FR_Class, ActnList, Menus, IBSQL;
...
TMiGrid = class(TCustomDBGrid);
  ...
  private
    ...
    function vEstadoSolicitud(_iEstado :Integer):Integer;
    function vValidaEstado(_iEstado:Integer):Integer;
...
uses UnitDmSolicitud, UnitDescObservacion, UnitDesreferencias,
  ...
  UnitConsultaProductos, UnitGlobalesCol, UnitdmColocacion, UnitdmGeneral;
...
begin
        ...
        if vValidaEstado(estado) >= 0 then
        begin
          DBente.KeyValue := vValidaEstado(estado);
...
//        if (estado >= 4) and (estado <> 9) and (estado <> 14) and (estado <> 18) then
        if (estado >= 4) and (estado <> 9) and (vValidaEstado(estado)< 0) then
        begin
        ...
        begin
          ...
          SQL.Clear;
          SQL.Add('select VALOR_MINIMO from "gen$minimos" where ID_MINIMO = 59');
          Open;
          _cMaxInforme := FieldByName('VALOR_MINIMO').AsInteger;

          SQL.Clear;
          SQL.Add('select VALOR_MINIMO from "gen$minimos" where ID_MINIMO = 0');
          Open;
          _cSalario := FieldByName('VALOR_MINIMO').AsInteger;
          _cMaxInforme := _cMaxInforme * _cSalario;
          SQL.Clear;
...
begin
        ...
        Label22.Caption := 'Créditos en ' + Empresa;
end;
...
begin
        ...
        with CDcapacidad do
        begin
          Filter := 'id_persona ='+ QuotedStr(CDcodeudoresid_persona.Value);
          Filtered := True;
          try
...
begin
        ...
        begin
          ...
          Filtered := False;
          Append;
          FieldValues['id_persona'] := QuotedStr(CDcodeudoresid_persona.Value);
...
begin
        ...
        if ingresos <> -1 then begin
          ...
          begin
            ...
            begin
              SQL.Clear;
              SQL.Add('update "col$solicitudanalista" set ESTUDIADA = 1,PENDIENTE = 0, ID_EMPLEADO_TRASLADO = :USUARIO where ID_SOLICITUD = :ID_SOLICITUD AND');
              SQL.Add('ES_VIGENTE = 1');
              ParamByName('ID_SOLICITUD').AsString := MKsolicitud.Text;
              if Role = 'ANALISTA' then
                 ParamByName('USUARIO').Clear
              else
                ParamByName('USUARIO').AsString := 'CREDITOS';

              {if rFavorable.Checked then
                 ParamByName('CONCEPTO').AsInteger := 1
              else
                 ParamByName('CONCEPTO').AsInteger := 0; }
            end
            else
            begin
              SQL.Clear;
              ...
              SQL.Clear;
              SQL.Add('insert into "col$solicitudanalista" values (ID_AGENCIA,:ID_SOLICITUD,:ID_EMPLEADO,:FECHAHORA,1,:USUARIO,1,0');//,:CONCEPTO)');
              ParamByName('ID_AGENCIA').AsInteger := Agencia;
              ParamByName('ID_SOLICITUD').AsString := MKsolicitud.Text;
...
begin
        ...
        try
          estado_solicitud := vEstadoSolicitud(DBente.KeyValue);
        except
        begin
...
//           BTnuevo.Click;
        ...
        begin
          if MessageDlg('Esta Seguro de Registrar el Analisis',mtInformation,[mbyes,mbno],0) = mryes then
             ...
             Exit;
          if (DBente.KeyValue = 2) or (JVvaloraprobado.Value > _cMaxInforme ) then
          begin
            if MessageDlg('Desea Registrar el Informe de Análisis de Crédito para Aprobación',mtInformation,[mbyes,mbno],0) = mryes then
            begin
...
var
   PorcentajeCapacidad : Currency;
begin
        PorcentajeCapacidad := ObtenerMinimo(dmGeneral.IBDatabase1, 99);
        JVdispago.Value := (PorcentajeCapacidad / 100) * JVdisponible.Value;
end;
...
begin
        ...
        filtro := 'id_persona = ' + QuotedStr(CDcodeudoresid_persona.Value) + ' and id_identificacion = ' + IntToStr(CDcodeudoresid_identificacion.Value);
        with CdFianzas do
        begin
...
function TFrmAnalisiscre.vEstadoSolicitud(_iEstado :Integer): Integer;
begin
        with DmSolicitud.IBSQL1 do
        begin
          Close;
          ...
          Transaction.StartTransaction;
          SQL.Clear;
          SQL.Add('select ID_ESTADO from "col$enteaprobador" where ID_ENTE_APROBADOR = :ID_ENTEAPROBADOR');
          ParamByName('ID_ENTEAPROBADOR').AsInteger := _iEstado;
          ExecQuery;
          Result := FieldByName('ID_ESTADO').AsInteger;
        end;
end;

function TFrmAnalisiscre.vValidaEstado(_iEstado:Integer): Integer;
var _iEstadoSol :Integer;
begin
        with DmSolicitud.IBSQL1 do
        begin
          Close;
          ...
          Transaction.StartTransaction;
          SQL.Clear;
          SQL.Add('select ID_ENTE_APROBADOR from "col$enteaprobador" where ID_ESTADO = :ID_ESTADO');
          ParamByName('ID_ESTADO').AsInteger := _iEstado;
          ExecQuery;
          if RecordCount = 0 then
            _iEstadoSol := -1
          else
            _iEstadoSol :=  FieldByName('ID_ENTE_APROBADOR').AsInteger;
          Result := _iEstadoSol;
        end;
end;
