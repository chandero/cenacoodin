...
type
  TfrmEntregaDetalles = class(TForm)
    ...
    imFotoJ: TImage;
    imFotoT: TImage;
    chkFechaActualizacion: TCheckBox;
    edFechaActualizacion: TStaticText;
    Label15: TLabel;
    edFechaActualizacionJ: TStaticText;
    GroupBox6: TGroupBox;
    chkFechaActualizacionJ: TCheckBox;
...
var
  ...
  xEdad:Boolean;
  xActualizacion:Boolean;
  xActualizacionJ:Boolean;

  vFechaActualizacion: TDate;
  vFechaActualizacionJ: TDate;
...
uses unitDmGeneral, unitGlobales, unitGlobalesCol, UnitdmColocacion,UnitGuardaImagen;
...
//        vJuvenil := 30000;
        ...
        xDeudas        :=False;
        xFianzas       :=False;
        xActualizacion := False;
        xActualizacionJ := False;        
        xApertura      :=False;
        xAperturaJ     := False;
...
// Verificación de Asociado y Requisitos /// QUITAR FOTO
        with IBQPersona do begin
          SQL.Clear;
          SQL.Add('select PRIMER_APELLIDO,SEGUNDO_APELLIDO,NOMBRE,EDUCACION,FOTO, FECHA_ACTUALIZACION from "gen$persona" where ID_IDENTIFICACION = :ID and');
...
//                xEducacion := False;
              ...
              vFechaActualizacion := FieldByName('FECHA_ACTUALIZACION').AsDateTime;
              if (DiasEntre(vFechaActualizacion, vFechaLimite) > 365 ) then
              begin
                xActualizacion := false;
                chkFechaActualizacion.Checked := false;
              end
              else
              begin
                xActualizacion := true;
                chkFechaActualizacion.Checked := true;                
              end;
              edFechaActualizacion.Caption := DateToStr(FieldByName('FECHA_ACTUALIZACION').AsDateTime);
              IBSQL1.Close;
              IBSQL1.SQL.Clear;
...
//
           ...
           IBSQL1.ParamByName('ANO').AsString := IntToStr(DBAnho);
...
begin
        ...
        if (xAportesAct and xJuvenil and xApertura
            and xDeudas and xFianzas and xRindediario and xActualizacion) then begin
...
//        ValidarAsociado;
        ...
        begin
           jpg := TJpegImage.Create;
           jpg.LoadFromStream(_rImagen.Foto);
           imFotoT.Picture.Bitmap.Assign(jpg);
           imFotoT.Repaint;
           FreeAndNil(jpg);
        end;

        _rimagen := _iImagen.ConsultaImagen(DBLCBTipoDocumentoJuvenil.KeyValue,EdDocumentoJuvenil.Text);
        if _rImagen.Foto.Size > 0 then
        begin
           jpg := TJpegImage.Create;
           jpg.LoadFromStream(_rImagen.Foto);
           imFotoJ.Picture.Bitmap.Assign(jpg);
           imFotoJ.Repaint;
           FreeAndNil(jpg);
        end;
        ...
        if (xAportesJ and xRindediarioJ and xJuvenilJ and xDeudas and xAperturaJ and xEdad and xActualizacionJ) then begin
            EdMensajesJ.Color := clWhite;
...
// Verificación de Asociado y Requisitos
        ...
        with IBQPersonaJ do begin
          SQL.Clear;
          SQL.Add('select PRIMER_APELLIDO,SEGUNDO_APELLIDO,NOMBRE,EDUCACION,FOTO,FECHA_NACIMIENTO,FECHA_ACTUALIZACION from "gen$persona" where ID_IDENTIFICACION = :ID and');
          SQL.Add('ID_PERSONA = :PERSONA');
          ParamByName('ID').AsInteger := DBLCBTipoDocumentoJuvenil.KeyValue;
          ParamByName('PERSONA').AsString := EdDocumentoJuvenil.Text;
          try
          Open;
           if RecordCount > 0 then begin
              ...
              vFechaActualizacionJ := FieldByName('FECHA_ACTUALIZACION').AsDateTime;
              edFechaActualizacionJ.Caption := DateToStr(FieldByName('FECHA_ACTUALIZACION').AsDateTime);
              EdEdad.Caption := IntToStr(Edad);
              if (DiasEntre(vFechaActualizacionJ, vFechaLimite) > 365) then
              begin
                  xActualizacionJ := false;
                  chkFechaActualizacionJ.Checked := false;
              end
              else
              begin
                  xActualizacionJ := true;
                  chkFechaActualizacionJ.Checked := true;
              end;
              if Edad < 14 then
                xEdad := True
              else
