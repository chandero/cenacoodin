...
uses
  ...
  FR_Class, JvLabel, Menus, IBSQL,JPEG;
...
uses UnitDmSolicitud, UnitAnalisisPago, UnitDesreferencias, UnitGlobalesCol,
  UnitCreditosFianzas, UnitDescObservacion, UnitInfGeneral, UnitImpresion,
  UnitMain, UnitConsultaProductos, unitGlobales,Zlib, UnitHabilitaFianza, UnitInformacionBancoldexEnte,UnitGuardaImagen;

{$R *.dfm}

procedure TFrmEnteAprobador.FormCreate(Sender: TObject);
begin
        if (not IBente.Database.Connected) then
        begin
            IBente.Database.Open;
            if (IBente.Transaction.InTransaction) then
               IBente.Transaction.Commit;
        end;
        IBente.Transaction.StartTransaction;
        IBente.Open;
        IBente.Last;
        DmSolicitud := TDmSolicitud.Create(self);
        if Id_EnteAprobador = 3 then
           CHverifica.Enabled := False;
        Label23.Caption := 'Creditos en ' + Empresa;
end;
...
//              JVtasaa.Value := SimpleRoundTo(BuscoTasaEfectivaMaximaDtfNueva(IBQuery4,fFechaActual) + FieldByName('PUNTOS_ADIC').AsFloat,-2);
        ...
        _timagenR := _tImagen.ConsultaImagen(id_identificacion,id_persona);
...
begin
        reporte;
        if (DBente.KeyValue = 1) and (CHverifica.Checked = False) then
        begin
           imprimir_reporte(frmMain.wpath+'reporte\repinformegerencia.frf');
           Exit;
        end;
        if CHverifica.Checked then
...
begin
        ...
        Try
          ...
          frreport1.SavePreparedReport(frmMain._sSession + '\' + IntToStr(Agencia) + Msolicitud.Text + '.frp');
        except
        end;
...
begin
        ...
        begin
          ...
          Transaction.StartTransaction;
          SQL.Clear;
          SQL.Add('select "col$solicitud".LINEA, "col$solicitud".ESTADO,"col$estadosolicitud".DESCRIPCION_ESTADO,"col$solicitud".VALOR_APROBADO from "col$solicitud"');
          SQL.Add('INNER JOIN "col$estadosolicitud" ON ("col$solicitud".ESTADO = "col$estadosolicitud".ID_ESTADO)');
          SQL.Add('where ID_SOLICITUD = :ID_SOLICITUD');
          ...
          begin
            _sDescripcionEstado := FieldByName('DESCRIPCION_ESTADO').AsString;
            if (FieldByName('VALOR_APROBADO').AsCurrency > Monto_EnteAprobador) and (FieldByName('LINEA').AsInteger <> 6) and (FieldByName('LINEA').AsInteger <> 12) and (FieldByName('LINEA').AsInteger <> 8) then
            begin
               ...
               end// FIN DEL BEGIN DEL EXEDE EL MONTO
            else
            begin
            ...
            if (FieldByName('ESTADO').AsInteger in [9]) or (vValidaEstado(FieldByName('ESTADO').AsInteger) >= 0) then
            begin
            ...
            begin
              MessageDlg('La Solicitud se Encuentra en Estado : ' + _sDescripcionEstado,mtInformation,[MBok],0);
              Msolicitud.SetFocus;
              Exit;
            end;
          end;
...
//        if (DBente.KeyValue = 2) and (CHverifica.Checked)  then  // comienzo verifica tipo ente aprobador
        if (CHverifica.Checked)  then  // comienzo verifica tipo ente aprobador
        begin
...
function TFrmEnteAprobador.vValidaEstado(_iEstado:Integer): Integer;
