...
begin
        ...
        begin
          ...
          begin
            ...
            IBSQL1.SQL.Add('sum("col$colocacion".VALOR_DESEMBOLSO)as VALOR_DESEMBOLSO,');
            ...
            IBSQL1.SQL.Add('group by "col$colocacion".ID_CLASIFICACION');
            ...
            begin
             ...
             begin
               ...
               Sg1.Cells[0,Fila] := '2';
               Sg1.Cells[1,Fila] := '60';
               Sg1.Cells[2,Fila] := CDprestamo.FieldByName('inversion').AsString; //CLASIFICACION
               ...
               Sg1.Cells[3,Fila] := Tp;//TIPO DE DOCUMENTOS
               Sg1.Cells[4,Fila] := Id;//NUMERO DOCUMNETO
               Sg1.Cells[5,Fila] := Dg;//DIGITO VERIFICACION OPCIONAL
              { with IBSQL1 do
               begin
                Close;
                SQL.Clear;
               ...
               Sg1.Cells[6,Fila] := Trim(FieldByName('PRIMER_APELLIDO').AsString);//PRIMER APELLIDO
               Sg1.Cells[7,Fila] := Trim(FieldByName('SEGUNDO_APELLIDO').AsString);//SEGUNDO APELLIDO
               _sNombre := Trim(FieldByName('NOMBRE').AsString);
               Sg1.Cells[8,Fila] := _sNombre;
               if Pos(' ', _sNombre) > 0 then
               begin
                   Sg1.Cells[8,Fila] := LeftStr(_sNombre,Pos(' ', _sNombre)-1);
                   Sg1.Cells[9,Fila] := RightStr(_sNombre,Length(_sNombre) - Pos(' ', _sNombre));
               end;

               if Tp = '31' then
                 Sg1.Cells[10,Fila] := Nm
               else
                 Sg1.Cells[10,Fila] := '';
               Sg1.Cells[11,Fila] := Dr;
               Sg1.Cells[12,Fila] := Dpto;
               Sg1.Cells[13,Fila] := Mn;
               Sg1.Cells[14,Fila] := FormatCurr('#0',CDprestamo.FieldByName('valor').AsCurrency);
...
begin
        ...
        if _iAno <> YearOf(fFechaActual) then
        begin
           DtFecha.DateTime := EncodeDate(_iAno,12,31);
