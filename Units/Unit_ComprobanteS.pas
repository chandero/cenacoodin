...
begin
  ...
  if vModificar then
     begin
      ...
      if descripcion ='' then
       if messagedlg('No ha digitado la descripci?n',mtError,[mbOk],0) = mrok then
        begin
      ...
      if List.Count = 0 then
       begin
       if messagedlg('No ha digitado ning?n movimiento',mtError,[mbOk],0) = mrok then
         exit;
       end;
      if edittotaldebito.Text <> edittotalcredito.Text then
       begin
       if messagedlg('El comprobante No est? cuadrado',mtError,[mbOk],0)= mrok then
         exit;
       end;
      if Actualizar then
          begin
