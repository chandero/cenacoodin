...
begin
        ...
        begin
          ...
          begin
             ...
             //Restriccion Para la renovación del Credivida
             ...
             begin
                _vfecha := FieldByName('FECHA').AsDateTime;
                _iAno := YearOf(_vfecha);
                _iDia := DayOf(_vfecha);
                _iMes := MonthOf(_vfecha);
                //_vfecha
                close;
                SQL.Clear;
                SQl.Add('update "col$datocredivida" set FECHA = :FECHA where CERTIFICADO = :CERTIFICADO');
                ParamByName('FECHA').AsDate := EncodeDate(_iAno+1,_iMes,_iDia); //FFechaActual + 365;
                ParamByName('CERTIFICADO').AsString := v_Certificado;
                ExecSQl;
             end;
          end
...
begin
      ...
      try
        ...
        begin
          ...
          SQL.Clear;
          SQL.Add('SELECT');
          SQL.Add('COL$CODIGOSPUCBASICOS.CODIGO');
          SQL.Add('FROM');
          SQL.Add('COL$CODIGOSPUCBASICOS');
          SQL.Add('WHERE');
          SQL.Add('COL$CODIGOSPUCBASICOS.ID_CODIGOPUCBASICO = 14');
          Open;
          _sCodigoCredivida := FieldByName('CODIGO').AsString;
          Close;
          sql.Clear;
         ...
         if tipo then
         begin
            auxiliar(_sCodigoCredivida,'',0,0,valor_credivida,False);
