...
uses
  ...
  IBDatabase, JvEdit, JvFloatEdit, DBClient, Grids, DBGrids, FR_Class;
...
type
  TfrmCertificaReteFuenteCap = class(TForm)
    ...
    IBQuery2: TIBQuery;
    CDCuentasMONTO: TCurrencyField;
    CDCuentasRETENCION: TCurrencyField;
    prReport1: TprReport;
...
//   ShowMessage(IBDatabase1.DatabaseName);
   ...
   with IBSQL1 do begin
     ...
     SQL.Add('"cap$maestro".DIGITO_CUENTA = "cap$maestrotitular".DIGITO_CUENTA and "cap$maestrotitular".NUMERO_TITULAR = 1)');
     ...
     while not Eof do begin
        ...
        try
          ...
          if Tipo = 1 then begin
             ...
             SaldoRetFueCon := SaldoRetFueCon + IBSQL3.FieldByName('RETEFUENTE').AsCurrency;
          end;
         except
          raise;
        end;
    ...
    if SaldoApo > 0 then begin
      ...
      CDCuentas.FieldByName('MONTO').AsCurrency := SaldoApo;
...

{
     ...
     begin
       CDCuentas.Append;
       CDCuentas.FieldByName('TIPO').AsString := 'Intereses Pagados y Causados';
       CDCuentas.FieldByName('MONTO').AsCurrency := FieldByName('valor').AsCurrency;
       CDCuentas.FieldByName('RETENCION').AsCurrency := 0;
       CDCuentas.Post;
     end;
     ...
     Close;
     SQL.Clear;
     SQL.Add('select SUM(RETENCION) AS VALOR from "dian$retefuente"');
     ...
     //CDCuentas.FieldByName('MONTO').AsCurrency := SaldoRetFue;
     //CDCuentas.Post;
...
// Colocaciones
     ...
     SQL.Add('where ID_IDENTIFICACION = :ID_IDENTIFICACION and ID_PERSONA = :ID_PERSONA and ID_ESTADO_COLOCACION NOT IN (4,5,10,12,13)');
     ParamByName('ID_IDENTIFICACION').AsInteger := DBLCBTipoIdentificacion.KeyValue;
     ParamByName('ID_PERSONA').AsString := NumeroDoc;
     ...
     try
      ...
      if SaldoCre > 0 then begin
        ...
        CDCuentas.FieldByName('MONTO').AsCurrency := SaldoCre;
        CDCuentas.FieldByName('RETENCION').AsCurrency := 0;
        CDCuentas.Post;
      end;
     except
...
// Intereses Recibidos
     ...
     try
      ...
      if SaldoIntRec > 0 then begin
        ...
        CDCuentas.FieldByName('MONTO').AsCurrency := SaldoIntRec;
        CDCuentas.FieldByName('RETENCION').AsCurrency := 0;
        CDCuentas.Post;
      end;
     except
...
//
   ...
   with prReport1 do begin
     ...
     Variables.ByName['CIUDADO'].AsString := Ciudad;
...
begin
        ...
        IBQuery2.Close;
        IBQuery2.Open;
        EdTasa.Value := IBQuery2.FieldByName('VALOR_MINIMO').AsFloat;
