...
begin
        ...
        begin
             ...
             ParamByName('ID_RETEFUENTE').AsInteger := id_retefuente;
             try
                Open;
        ...
        begin
            ...
            EdTasaRfte.Caption := FormatCurr('#0.00%',TasaR);
...
//              IBConsulta.SQL.Add(':"INTERES",:"RETENCION")');
          ...
          //if (Agencia = 1) and (DBLCBTipoCaptacion.KeyValue = 2) then
          //   CmdAplicar.Click
          //else
        ...
        begin
           ...
           Registros := CDStemp.RecordCount;
          {
           ...
           begin
              ...
              frmProgreso.InfoLabel := CDStempID_TIPO_CAPTACION.AsString + '-' + Format('%.2d',[CDStempID_AGENCIA.Value]) + '-' +
...
//Grabar Interes en Extracto
              ...
              begin
                ...
                try
                  ...
                  begin
                   MessageDlg('Error Grabando Retención de Captación:' + Format('%.7d',[CDStempNUMERO_CUENTA.AsInteger]),mtError,[mbok],0);
                   IBPagar.Transaction.RollbackRetaining;
                   Exit;
                  end
                except
                  MessageDlg('Error Grabando Intereses de Captación:' + Format('%.7d',[CDStempNUMERO_CUENTA.AsInteger]),mtError,[mbok],0);
                  IBPagar.Transaction.RollbackRetaining;
                  Exit;
                end;
              end;
              ...
              if certificado then
                ...
                begin
                  ...
                  try
                    ...
                    if IBPagar.RowsAffected < 1 then
                    begin
                     MessageDlg('Error Grabando Intereses de Captación:' + Format('%.7d',[CDStempNUMERO_CUENTA_ABONO.AsInteger]),mtError,[mbok],0);
                     IBPagar.Transaction.RollbackRetaining;
                     Exit;
                    end
                  except
              ...
              if certificado then
                ...
                begin
                  ...
                  try
                    ...
                    if IBPagar.RowsAffected < 1 then
                    begin
                     MessageDlg('Error Grabando Retención de Captación:' + Format('%.7d',[CDStempNUMERO_CUENTA_ABONO.AsInteger]),mtError,[mbok],0);
...
// Grabo Detalle de la Retención
              ...
              try
               ...
               begin
                IBPagar.Transaction.Rollback;
                MessageDlg('Error Grabando en tabla de retenciones captacion:' + Format('%.7d',[CDStempNUMERO_CUENTA.AsInteger]),mtError,[mbcancel],0);
                Exit;
               end
              except
                IBPagar.Transaction.Rollback;
...
// En Caso de Certificado, Marcar la tabla de liquidación
            ...
            begin
              ...
              try
               ...
               begin
                IBPagar.Transaction.Rollback;
                MessageDlg('Error Actualizando Tabla de Liquidación Captación:' + Format('%.7d',[CDStempNUMERO_CUENTA.AsInteger]),mtError,[mbcancel],0);
                Exit;
               end
              except
                IBPagar.Transaction.Rollback;
...
// Grabar maestro fecha ultimo pago
              ...
              try
               ...
               begin
                IBPagar.Transaction.Rollback;
                MessageDlg('Error Actualizando Fecha Ultimo Pago Captación:' + Format('%.7d',[CDStempNUMERO_CUENTA.AsInteger]),mtError,[mbcancel],0);
                Exit;
               end
              except
                IBPagar.Transaction.Rollback;
