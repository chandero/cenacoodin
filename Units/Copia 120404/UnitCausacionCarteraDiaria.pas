                  ...
                  Dias := Dias + IBQuery1.FieldByName('AMORTIZA_INTERES').AsInteger;
{
                if IBQuery1.FieldByName('ID_ESTADO_COLOCACION').AsInteger = 2 then
                  Dias := DiasEntreFechas(IncDay(FechaInicial),FechaFinal,IBQuery1.FieldByName('FECHA_DESEMBOLSO').AsDateTime + IBQuery1.FieldByName('DIAS_PAGO').AsInteger)
                else
                  begin
                    Dia := DayOf(IBQuery1.FieldByName('FECHA_DESEMBOLSO').AsDateTime) + IBQuery1.FieldByName('DIAS_PAGO').AsInteger;
...
{                Dia := DayOf(IBQuery1.FieldByName('FECHA_DESEMBOLSO').AsDateTime) + IBQuery1.FieldByName('DIAS_PAGO').AsInteger;
                if IBQuery1.FieldByName('ID_ESTADO_COLOCACION').AsInteger = 2 then
                 ...
                 begin
                  if TryEncodeDate(YearOf(IBQuery1.FieldByName('FECHA_INTERES').AsDateTime),MonthOf(IBQuery1.FieldByName('FECHA_INTERES').AsDateTime),Dia,NFecha) then
                    DiasMora := DiasEntreFechas(IncDay(NFecha),FechaFinal,IBQuery1.FieldByName('FECHA_DESEMBOLSO').AsDateTime + IBQuery1.FieldByName('DIAS_PAGO').AsInteger)
                  else
...
//                if IBQuery1.FieldByName('TIPOC_INTERES').AsString = 'V' then
//                  DiasMora := DiasMora - IBQuery1.FieldByName('AMORTIZA_INTERES').AsInteger;
...
begin
          ...
          while not CDReal.Eof  do begin
              ...
              IBSQL10.ParamByName('MATRICULA').AsInteger := CDReal.FieldByName('MATRICULA');
              IBSQL10.ExecQuery;

              frmPantallaProgreso.Position := CDReal.RecNo;
              frmPantallaProgreso.InfoLabel := CDReal.FieldByName('MATRICULA').AsString + '-' + IBSQL10.FieldByName('ID_COLOCACION').AsString;
              Application.ProcessMessages;
              ...
              if SaldoCapital < SaldoGarantia then
                 ...
                 begin
                   ...
                   IBQRealAct.ParamByName('ID_COLOCACION').AsString := IBSQL10.FieldByName('ID_COLOCACION').AsString;
                   IBQRealAct.ParamByName('VALOR_GARANTIA').AsCurrency := SaldoGarantia;
                   IBQRealAct.ParamByName('GARANTIA_DESCONTADA').AsCurrency := SaldoCapital - SaldoGarantia;
                   IBQRealAct.ExecSQL;
                   IBQRealAct.Transaction.Commit;
...
//          end;
         ...
         frmPantallaProgreso.Cerrar;
         IBSQL3.Transaction.Commit;
         MessageDlg('Provisi?n Terminada con Exito!!',mtInformation,[mbok],0);
       // Fin Actualizaci?n
end;
