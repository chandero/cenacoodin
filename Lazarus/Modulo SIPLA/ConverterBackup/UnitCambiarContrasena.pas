...
uses
  ...
  Dialogs, IBServices, StdCtrls, ExtCtrls, Buttons, IBSQL;
...
begin
     ...
     if (Trim(EdPasabordo.Text) = '') or (Trim(EdRePasabordo.Text ) = '') then
     begin
        MessageDlg('Las contraseñas digitadas no contienen caracteres',mtError,[mbcancel],0);
     ...
     if Length(EdPasabordo.Text) < 5 then
     begin
        MessageDlg('Las contraseña debe tener como mino 5 caracteres',mtError,[mbcancel],0);
        EdPasabordo.SetFocus;
        Exit;
     end;
