...
function ObtenerDiasMoraAFecha(Agencia:Integer;Colocacion:string;IBSQL1:TIBSQL;FechaCorte:TDate) : Integer;
...
procedure CrearTablaLiquidacionNuevaCuota(TableName:String;vDesembolso:Currency;ValorCuota:Currency;
                               FechaDesembolso:TDate;TasaE:Single;PuntosAdicionales:Single;
                               AmortizaCapital:Integer;
                               AmortizaInteres:Integer;Plazo:Integer;
...
//Busco Costas
     ...
     // Calcular Fechas a Liquidar
        ...
        // MyCuotasLiq.CapitalHasta :=  CalculoFecha(FechaProxK, -_diasGracia);
...
//        FechaPagoI := CalculoFecha(FechaPagoI,DiasProrroga);
        ...
        // MyCuotasLiq.InteresesHasta := CalculoFecha(FechaPagoI, -_diasGracia);
...
//          Pagos := Plazo div (AmortizaCapital div 30);
          ...
          begin
            ...
            begin
                ...
                SQL.Add(' set CAPITAL_A_PAGAR = :"CAPITAL",INTERES_A_PAGAR = :"INTERES" where FECHA_A_PAGAR = :"FECHA" and ID_COLOCACION = :"ID_COLOCACION"');
                ParamByName('ID_COLOCACION').AsString := TableName;
                ParamByName('FECHA').AsDate := FechaK;
        ...
        { Proceso Capital }
          ...
          begin
            ...
            begin
                SQL.Clear;
                SQL.Add('update "col$tablaliquidacion" ');
                SQL.Add(' set CAPITAL_A_PAGAR = :"CAPITAL" where FECHA_A_PAGAR = :"FECHA" and ID_COLOCACION = :"ID_COLOCACION"');
                ParamByName('ID_COLOCACION').AsString := TableName;
                ParamByName('CAPITAL').AsCurrency := Capital;
                ParamByName('FECHA').AsDate := FechaK;
                ExecSQL;
...
//    Plazo := Plazo div 30;
    ...
    _vQuery := TIBQuery.Create(nil);

    _vTransaction.DefaultDatabase := dmGeneral.IBDatabase1;
    _vQuery.Database := dmGeneral.IBDatabase1;
    _vQuery.Transaction := _vTransaction;

    _vTransaction.StartTransaction;
...
//    Pagos := Plazo div (Amortiza div 30);
    ...
    begin
        SQL.Clear;
        SQL.Add('DELETE FROM "col$tablaliquidacion" t WHERE t.ID_COLOCACION = :ID_COLOCACION and t.FECHA_A_PAGAR > :FECHA_A_PAGAR');
        ParamByName('ID_COLOCACION').AsString := TableName;
        ParamByName('FECHA_A_PAGAR').AsDate := FechaDesembolso;
        ExecSQL;
    end;
    ...
    begin
        ...
        begin
           ...
           ParamByName('ID_AGENCIA').AsInteger := Agencia;
           ParamByName('ID_COLOCACION').AsString := TableName;
           ParamByName('CUOTA_NUMERO').AsInteger := vCuota + B;
           ParamByName('FECHA').AsDate := FechaK;
           ParamByName('CAPITAL').AsCurrency := 0;
           ParamByName('INTERES').AsCurrency := 0;
           ParamByName('PAGADA').AsInteger := 0;
    ...
    begin
      ...
      { Proceso Interes }
          ...
          begin
             SQL.Clear;
             SQL.Add('select * from "col$tablaliquidacion" ');
             SQL.Add(' where FECHA_A_PAGAR = :"FECHA" and ID_COLOCACION = :"ID_COLOCACION"');
             ParamByName('ID_COLOCACION').AsString := TableName;
             ParamByName('FECHA').AsDate := FechaI;
             Open;
             if RecordCount > 0 then
             ...
             Saldo := Saldo - Saldo1;
               SQL.Clear;
               SQL.Add('update "col$tablaliquidacion" ');
               SQL.Add(' set INTERES_A_PAGAR = :"INTERES" where FECHA_A_PAGAR = :"FECHA" and ID_COLOCACION = :"ID_COLOCACION"');
               ParamByName('ID_COLOCACION').AsString := TableName;
               ParamByName('INTERES').AsCurrency := Interes;
               ParamByName('FECHA').AsDate:= FechaI;
               ExecSQL;
...
begin
  ...
  with IBQ do begin
   ...
   CrearTablaLiquidacionPTmp(Consec,vDesembolso,ValorCuota,
            ...
            AmortizaInteres,Plazo,TipoCuota,Modalidad,vPeriodoGracia);
...
//        Dia := DayOf(IBSQL1.FieldByName('FECHA_DESEMBOLSO').AsDateTime) + IBSQL1.FieldByName('DIAS_PAGO').AsInteger;
        ...
        if (IBSQL1.FieldByName('ID_LINEA').AsInteger <> 6) and (IBSQL1.FieldByName('ID_ESTADO_COLOCACION').AsInteger = 0) then
          begin
        ...
        IBQvalidar.Close;
        IBQvalidar.SQL.Clear;
        IBQvalidar.SQL.Add('SELECT FIRST 1 * FROM COL_PERIODO_GRACIA WHERE ID_COLOCACION = :ID_COLOCACION AND ESTADO < 9 ORDER BY FECHA_REGISTRO DESC');
        IBQvalidar.ParamByName('ID_COLOCACION').AsString := Colocacion;
        IBQvalidar.Open;
        if IBQvalidar.RecordCount > 0 then
        begin
           // _fechaInicial := IBQvalidar.FieldByName('FECHA_REGISTRO').AsDateTime;
           _fechaInicial := CalculoFecha(IBQvalidar.FieldByName('FECHA_INTERES').AsDateTime, Amortiza);
           _diasGracia := IBQvalidar.FieldByName('DIAS').AsInteger;
           _fechaFinal := CalculoFecha(_fechaInicial, _diasGracia);
...
begin
        ...
        if Tipo = 'V' then
           Fecha := CalculoFecha(Fecha,Amortiza);
        FechaDes := CalculoFecha(_dFechaD,_iDiasPago);
        Dia := DayOf(FechaDes);
        if (_iLinea <> 6) and (_iEstado = 0) then
          begin
            if TryEncodeDate(YearOf(Fecha),MonthOf(Fecha),Dia,Fecha1) then
              Fecha := Fecha1;
          end;
