...
Uses Graphics, Forms, StdCtrls, DBCtrls, Math, DateUtils, IB, IBSQL ,IBQuery,IBStoredProc, IBDataBase, Messages,SysUtils,DB,DBGrids,Windows,Controls, StrUtils,Classes,Dialogs, winspool, Printers,
     Unit_DmComprobante,JvStringGrid,DBClient,IdIcmpClient, IdHTTP;
...
type
   TImagen = Record
    Foto:TMemoryStream;
    Huella:TMemoryStream;
    Firma:TMemoryStream;
end;

type
   TTarjetaDebito = Record
     Id: Integer;
     Tarjeta: String;
end;
...
procedure  validarUltimaActualizacion(Id:Integer; Numero:String);
...
procedure Empleado;
procedure LogBase(_dLog:TClientDataSet);
function ReCalculoSaldosGenerales:Boolean;
function ReCalculoSaldosGeneralesNew:Boolean;
...
function ParametroRep(_iIdParametro:Integer):string;
function CrearInsert(_stabla:string;_dDataset:TClientDataSet):string;
function ValidaIncapacitado(_IdIdentificacion:Integer;_sIdPersona:string):string;
function PingHost(Host: string): Boolean;
function GetURLAsString(const aUrl:String):String;
function GetMovCajaId():Integer;
function GetCapExtId():Integer;
function GetColExtId():Integer;
function GetMovColId():Integer;
function GetMovOtrosId():Integer;
...
// Variable detalles
 ...
 // Variables Entrega de Ayudas
  _iPeriodo: Integer;
  _cValorAportes :Currency;
 // Variable proceso de Creación de Notas
  _iCreacionNota :Integer;
 //Variables de Base de datos Imagennes
  _sServerImagen :string;
...
function ObtenerConsecutivoVirtual(id:Integer): LongInt;
...
function ObtenerMinimo(IBDatabase1:TIBDatabase; minimo:Integer): Currency;
...
begin
   ...
   _query.SQL.Clear;
   _query.SQL.Add('SELECT GEN_ID("GEN_caj$movimiento_ID",1) AS ID FROM rdb$database');
   _query.Open;
   _id := _query.FieldByName('ID').AsInteger;
   _transaction.Commit;
   result := _id;
end;
...
begin
   ...
   _query.SQL.Clear;
   _query.SQL.Add('SELECT GEN_ID("GEN_cap$extracto_ID",1) AS ID FROM rdb$database');
   _query.Open;
   _id := _query.FieldByName('ID').AsInteger;
   _transaction.Commit;
   result := _id;
...
begin
   ...
   _query.SQL.Clear;
   _query.SQL.Add('SELECT GEN_ID("GEN_col$extracto_ID",1) AS ID FROM rdb$database');
   _query.Open;
   _id := _query.FieldByName('ID').AsInteger;
   _transaction.Commit;
   result := _id;
...
begin
   ...
   _query.SQL.Clear;
   _query.SQL.Add('SELECT GEN_ID("GEN_caj$movimientocol_ID",1) AS ID FROM rdb$database');
   _query.Open;
   _id := _query.FieldByName('ID').AsInteger;
   _transaction.Commit;
   result := _id;
...
begin
   ...
   _query.SQL.Clear;
   _query.SQL.Add('SELECT GEN_ID("GEN_caj$recaudos_ID",1) AS ID FROM rdb$database');
   _query.Open;
   _id := _query.FieldByName('ID').AsInteger;
   _transaction.Commit;
   result := _id;
...
begin
        with dmGeneral.IBSQL1 do
         begin
           ...
           sql.Clear;
           sql.Add('SELECT p.FECHA_ACTUALIZACION, ( p.PRIMER_APELLIDO || '' '' || p.SEGUNDO_APELLIDO || '' '' || p.NOMBRE ) AS ASOCIADO FROM "gen$persona" p');
           sql.Add('where p.ID_IDENTIFICACION = :ID_IDENTIFICACION and p.ID_PERSONA = :ID_PERSONA');
           ParamByName('ID_IDENTIFICACION').AsInteger := Id;
           ParamByName('ID_PERSONA').AsString := Numero;
           ExecQuery;
           Fecha := FieldByName('FECHA_ACTUALIZACION').AsDate;
           Asociado := FieldByName('ASOCIADO').AsString;
           if DaysBetween(Fecha, fFechaActual) > 360 then
           begin
               ShowMessage('El Asociado ' + Asociado + ' debe Acercarce a Actualizar Datos!!!');
           end;
...
begin
        n := 0;
        Cuenta := FormatFloat('0000000',StrToInt(Cuenta));
...
var
 ...
 lbuf:integer;
begin
 PrinterName1 := 'Generic / Text Only';
...
begin
       ...
       while True do
        with IBSQL1 do begin
         ...
         try
          SQL.Clear;
          SQL.Add('select * from "gen$consecutivos" where "gen$consecutivos"."ID_CONSECUTIVO" = :ID_CONSECUTIVO');
          ParamByName('ID_CONSECUTIVO').AsInteger := id;
          ExecQuery;
          Consecutivo := FieldByName('CONSECUTIVO').AsInteger;
          Close;
          Consecutivo := Consecutivo + 1;
          SQL.Clear;
          SQL.Add('update "gen$consecutivos" set "gen$consecutivos"."CONSECUTIVO" = :"CONSECUTIVO" ');
          SQL.Add(' where "gen$consecutivos"."ID_CONSECUTIVO" = :ID_CONSECUTIVO');
          ParamByName('ID_CONSECUTIVO').AsInteger := id;
          ParamByName('CONSECUTIVO').AsInteger := Consecutivo;
          ExecQuery;
          Transaction.Commit;
       ...
       Result := 0;
        with IBSQL1 do begin
         ...
         if Transaction.InTransaction then
          ...
          Transaction.StartTransaction;
          SQL.Clear;
          SQL.Add('select * from "gen$minimos" where "gen$minimos"."ID_MINIMO" = :ID_MINIMO');
          ParamByName('ID_MINIMO').AsInteger := minimo;
          ExecQuery;
          Valor := FieldByName('VALOR_MINIMO').AsCurrency;
          Close;
          Transaction.Commit;
        end;
        Result := Valor;
...
begin
        ...
        begin
          ...
          SQL.Clear;
          SQL.Add('SELECT');
          SQL.Add('"gen$parametrorep".PARAMETRO');
          SQL.Add('FROM');
          SQL.Add('"gen$parametrorep"');
          SQL.Add('WHERE');
          SQL.Add('"gen$parametrorep".ID = :ID');
          ParamByName('ID').AsInteger := _iIdParametro;
          ExecQuery;
          Result := FieldByName('PARAMETRO').AsString;
          Close;
        end;
        FreeAndNil(IBSQL1);
        FreeAndNil(IBTValida);
end;
function CrearInsert(_stabla:string;_dDataset:TClientDataSet):string;
var
        _sSql :string;
        ...
        _sComodin :string;
begin
          while not _dDataset.Eof do
          begin
            _sSql := _stabla;
            for i := 0 to _dDataset.Fields.Count -1 do
            begin
               if i = _dDataset.Fields.Count -1 then
                  _sComodin := ');'
               else
                  _sComodin := ',';
               try
                _sSql := _sSql + QuotedStr(_dDataset.Fields[i].Value) + _sComodin
               except
                 _sSql := _sSql + 'NULL' + _sComodin;
               end;
            end;
            _sSqlResult := _sSqlResult + _sSql;
            _dDataset.Next;
          end;
          Result := _sSqlResult;
end;
...
begin
        ...
        begin
          ...
          SQL.Add('  :TIPO,');
          SQL.Add('  :OPERACION)');
          ParamByName('IDEMPLEADO').AsString := _dLog.FieldByName('IDEMPLEADO').AsString;
          ParamByName('FECHADIA').AsDateTime := _dLog.FieldByName('FECHADIA').AsDateTime;
          ParamByName('TIPO').AsInteger := _dLog.FieldByName('TIPO').AsInteger;
          ParamByName('OPERACION').AsString := _dLog.FieldByName('OPERACION').AsString;
          ExecQuery;
          Transaction.Commit;
          Transaction.StartTransaction;
        end;
...
begin
        ...
        begin
          ...
          SQL.Clear;
          SQL.Clear;
          SQL.Add('select ESTADO,LOG FROM "gen$incapacitado" where ID_PERSONA = :IDPERSONA and ID_IDENTIFICACION = :IDIDENTIFICACION');
          ParamByName('IDIDENTIFICACION').AsInteger := _IdIdentificacion;
          ParamByName('IDPERSONA').AsString := _sIdPersona;
          ExecQuery;
          if FieldByName('ESTADO').AsInteger = 1 then
             Result := FieldByName('LOG').AsString
          else
             Result := '';
        end;
        FreeAndNil(IBSQL1);
