...
begin
    ...
    While FechaF1 <= FechaF2 do
     begin
       ...
       AF^.Devuelto := False;
       Fecha := FechaF1;
       Fecha := RecodeDay(Fecha,Dayof(EndOfAMonth(YearOf(Fecha),MonthOf(Fecha))));
       if Int(Fecha) > Int(FechaF1) then
          Fecha := FechaF2;
       AF^.Vencida := True;
       AF^.Devuelto := False;
       AF^.FechaInicial := FechaF1;
...
//          not (((M=MM) and (D=30) and (DD=30))) then
         FechasLiq.Add(AF);
       FechaF1 := IncDay(Fecha);
     end;
end;
...
var     IBSQL :TIBQuery;
        ...
        puntos :string;
begin
        IBSQL := TIBQuery.Create(Application);
