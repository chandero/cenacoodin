...
// Busco Codigo Contable
              ...
              try
                ...
                if IBQuery2.RecordCount > 0 then
                   Codigo := IBQuery2.FieldByName('COD_CAPITAL_CP').AsString;
              except
                Transaction.Rollback;
                raise;
              end;
              ...
              IBQuery2.Close;
              IBQuery2.SQL.Clear;
              IBQuery2.SQL.Add('SELECT NIIF FROM CON$PARALELO WHERE COLGAAP = :CODIGO');
              IBQuery2.ParamByName('CODIGO').AsString := Codigo;
              IBQuery2.Open;
              try
                IBQuery2.Open;
                if (IBQuery2.RecordCount > 0) then
                   CodigoNiif := LeftStr(IBQuery2.FieldByName('NIIF').AsString,6)
              except
                Transaction.Rollback;
                raise;
              end;
...
// Fin Codigo Contable
              Lineas.Codigo_Contable := CodigoNiif;
...
{              if FechaCorte > FechaVencimiento then
              ...
              if (StrToInt(Lineas.Morosidad) < 31) then Lineas.Morosidad := '0';
...
{                IBGarReal.SQL.Clear;
                IBGarReal.Close;
                IBGarReal.SQL.Add('SELECT');
                IBGarReal.SQL.Add('sum("col$datogarantia".CUENTAS_DE_ORDEN) AS TOTAL');
                IBGarReal.SQL.Add('FROM');
                IBGarReal.SQL.Add('"col$garantiacol"');
                IBGarReal.SQL.Add('INNER JOIN "col$datogarantia" ON ("col$garantiacol".MATRICULA = "col$datogarantia".MATRICULA)');
                IBGarReal.SQL.Add('WHERE');
                IBGarReal.SQL.Add('"col$garantiacol".ID_COLOCACION = :ID_COLOCACION');
                IBGarReal.ParamByName('ID_COLOCACION').AsString := FieldByName('ID_COLOCACION').AsString;
                IBGarReal.ExecQuery;
                _cValorGarantia := IBGarReal.FieldByName('TOTAL').AsCurrency;

                IBQuery2.Close;
                IBQuery2.SQL.Clear;
                IBQuery2.SQL.Add('SELECT');
                IBQuery2.SQL.Add('SUM("col$colocacion".VALOR_DESEMBOLSO) AS DESEMBOLSO');
                IBQuery2.SQL.Add('FROM');
                IBQuery2.SQL.Add('"col$garantiacol"');
                IBQuery2.SQL.Add('INNER JOIN "col$colocacion" ON ("col$garantiacol".ID_AGENCIA = "col$colocacion".ID_AGENCIA)');
                IBQuery2.SQL.Add('AND ("col$garantiacol".ID_COLOCACION = "col$colocacion".ID_COLOCACION)');
                IBQuery2.SQL.Add('WHERE');
                IBQuery2.SQL.Add('"col$colocacion".ID_ESTADO_COLOCACION IN (0,1,2) AND');
                IBQuery2.SQL.Add('"col$garantiacol".MATRICULA = :MATRICULA');
                IBQuery2.ParamByName('MATRICULA').AsString := IBGarReal.FieldByName('MATRICULA').AsString;
                IBQuery2.Open;
                try
                Lineas.Valor_Garantia := CurrToStr(SimpleRoundTo((_cDesembolso * (IBGarReal.FieldByName('CUENTAS_DE_ORDEN').AsCurrency / IBQuery2.FieldByName('DESEMBOLSO').AsCurrency)),0));
                Except
//                  Memo1.Lines.Add(FieldByName('ID_COLOCACION').AsString);
