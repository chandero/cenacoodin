...
uses
  ...
  IBDatabase, JvBlinkingLabel, DBXpress, FMTBcd, SqlExpr, Provider;
type
  Deuda = record
    Valor:Currency;
    Dias:Integer;
...
type Registro2 = Record     // Maestro Cifin
    TRegistro:string; // * 1 N
    TIden:string; // * 2 N
...
uses UnitPantallaProgreso, UnitGlobalesCol, UnitGlobales, UnitdmGeneral;
...
begin
        ...
        PlanoEncDataClientes.SaveToFile('c:/CL' + CodigoData + Nit + Format('%.4d',[YearOf(DTfecha.Date)])+ Format('%.2d',[monthOf(DTfecha.Date)]) + Format('%.2d',[DayOf(DTfecha.Date)]) + '.TXT');
        ...
        PlanoEncDataCartera.SaveToFile('c:/PTCAC' + CodigoData + Format('%.4d',[YearOf(DTfecha.Date)])+ Format('%.2d',[monthOf(DTfecha.Date)]) + Format('%.2d',[DayOf(DTfecha.Date)]) + '.TXT');
...
begin
        ...
        with IBQuery11 do begin
           ...
           while not CDDatos.Eof do begin
             ...
             if (CDDatos.FieldByName('ID_PERSONA').AsString = 'E363450') then
               Nit := '363450';
...
begin
        ...
        with IBQuery11 do begin
           ...
           while not CDDatos.Eof do begin
             ...
             case CDDatos.FieldByName('ID_AGENCIA').AsInteger of
               1 : LineaMCD.Oficina := '                         OCAÑA';
               2 : LineaMCD.Oficina := '                        ABREGO';
               3 : LineaMCD.Oficina := '                    CONVENCION';
               4 : LineaMCD.Oficina := '                     AGUACHICA';
             end;
             ...
             PlanoMovDataCartera.Add(LineaMCD.TpIden + LineaMCD.Identificacion + LineaMCD.Numero + LineaMCD.Nombre + LineaMCD.STitular +
                              ...
                              LineaMCD.Tdc + LineaMCD.TpMon + LineaMCD.TpGar + LineaMCD.Califi + LineaMCD.PIncump + LineaMCD.EdMora + LineaMCD.VInicial +
                              ...
                              LineaMCD.Email + LineaMCD.Celular + LineaMCD.SDestino + LineaMCD.Blanco);
...
// Sumatoria Numeros de identificacion
      LineaFinDataClientes := LineaFinDataClientes + StringReplace(Format('%18s',[CURRToStr(sumaidentificacion)]),' ','0',[rfReplaceAll]);
...
begin
        ...
        PlanoEncCifin.SaveToFile('c:/CSC' + CodigoCifin + MesCorteAct + '.TXT');
        ...
        with IBQuery11 do begin
           ...
           while not CDDatos.Eof do begin
             ...
             PlanoMovCifin.Add(LineaC.TRegistro + LineaC.TIden + LineaC.Identificacion +
                                  LineaC.Nombre + LineaC.Reserva + LineaC.Numero + LineaC.CodSuc +
