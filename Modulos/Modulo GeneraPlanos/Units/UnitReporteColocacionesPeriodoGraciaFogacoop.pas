unit UnitReporteColocacionesPeriodoGraciaFogacoop;
...
type
  TfrmReporteColocacionesPeriodoGraciaFogacoop = class(TForm)
    Label1: TLabel;
    CBMeses: TComboBox;
    IBQuery1: TIBQuery;
    IBQuery2: TIBQuery;
...
var
  frmReporteColocacionesPeriodoGraciaFogacoop: TfrmReporteColocacionesPeriodoGraciaFogacoop;
  FechaCorte:TDate;
  _fFechaAporte :TDate;
...
procedure TfrmReporteColocacionesPeriodoGraciaFogacoop.CmdCerrarClick(Sender: TObject);
begin
        Close;
end;

procedure TfrmReporteColocacionesPeriodoGraciaFogacoop.CmdProcesarClick(Sender: TObject);
var
Dia:Integer;
NRuta:string;
begin
...
procedure TfrmReporteColocacionesPeriodoGraciaFogacoop.Cartera;
var 
    Dia:Integer;
    Total:Integer;
    Lineas:TColocacion;
...
begin
    ...
    if SD1.Execute then
    begin
     AssignFile(Archivo, SD1.FileName);
    ...
    with IBQuery1 do begin
     ...
     SQL.Add('  cpge.CAUSADO AS CAUSADO_GRACIA,');
     ...
     SQL.Add('  cpge.PINTERES AS PINTERES_GRACIA,');
     ...
     SQL.Add('  LEFT JOIN COL_PERIODO_GRACIA_EXTRACTO cpge ON cpge.ID_COLOCACION = "col$causaciondiaria".ID_COLOCACION AND cpge.FECHA_CORTE = "col$causaciondiaria".FECHA_CORTE');
...
//Fin Cuota
              ...
              if (FieldByName('CAUSADO_GRACIA').AsCurrency <> 0) then
                Lineas.Saldo_Interes := FormatCurr('#0', FieldByName('CAUSADOS').AsCurrency + FieldByName('CAUSADO_GRACIA').AsCurrency)
              else
              ...
              if (FieldByName('PINTERES_GRACIA').AsCurrency <> 0) then
                Lineas.Provision_Interes := FormatCurr('#0',FieldByName('PINTERES').AsCurrency + FieldByName('PINTERES_GRACIA').AsCurrency)
              else
...
procedure TfrmReporteColocacionesPeriodoGraciaFogacoop.FormShow(Sender: TObject);
begin
        EdAno.Value := YearOf(Date);
        CBMeses.ItemIndex := MonthOf(Date);
end;

function TfrmReporteColocacionesPeriodoGraciaFogacoop.CalculoAportes(Agencia:Integer;id: Integer; pr: string): Currency;
var
