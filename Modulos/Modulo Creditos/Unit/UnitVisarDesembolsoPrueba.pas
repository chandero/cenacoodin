unit UnitVisarDesembolsoPrueba;
...
uses
  ...
  IBDatabase, strutils, Fr_Dbset;

type
  TfrmVisarDesembolsoPrueba = class(TForm)
    Panel1: TPanel;
    GroupBox1: TGroupBox;
    Label18: TLabel;
    Label1: TLabel;
...
var
  frmVisarDesembolsoPrueba: TfrmVisarDesembolsoPrueba;
  colocacion : string;
  vcolocacion : string;
...
uses UnitGlobales, UnitGlobalesCol, UnitDmGeneral, UnitdmColocacion,UnitBuscarColocacion, UnitTablaPagoColocacion, UnitConsultaGarantias,
  UnitDmSolicitud, UnitImpresion, unitMain;


procedure TfrmVisarDesembolsoPrueba.EdNumeroColocacionExit(Sender: TObject);
...
begin
        ...
        begin
                ...
                begin
                      ...
                      begin
                          MessageDlg('Esta Colocaci?n no se puede Visar',mtError,[mbOK],0);
           ...
           with IBSQL1 do begin
              ...
              try
                ...
                if RecordCount < 1 then begin
                   MessageDlg('Parametro de Salario M?nimo, no esta actualizado'+#13+
                              'Por Favor Consulte con Sistemas'+#13+
                              'Envie la Colocaci?n a Visado',mtError,[mbOk],0);
//                   Transaction.Commit;
                   Exit;
                end;
                ...
                MessageDlg('Error al Consultar Salario M?nimo''Por Favor Consulte con Sistemas'+#13+
                              'Envie la Colocaci?n a Visado',mtError,[mbOk],0);
                raise;
                Exit;
              end;
              ...
              try
                ExecQuery;
                if RecordCount < 1 then begin
                   MessageDlg('Parametro de Veces Salarios M?nimos para No Visado, no esta actualizado'+#13+
                              'Por Favor Consulte con Sistemas'+#13+
                              'Envie la Colocaci?n a Visado',mtError,[mbOk],0);
//                   Transaction.Commit;
                   Exit;
                ...
                Transaction.Rollback;
                MessageDlg('Error al Consultar Veces Salario M?nimo para No Visado''Por Favor Consulte con Sistemas'+#13+
                              'Envie la Colocaci?n a Visado',mtError,[mbOk],0);
                raise;
                Exit;
              end;
        ...
        begin
           ...
           begin
              CmdVisar.Enabled := False;
              CmdAnular.Enabled := False;
              MessageDlg('No tiene privilegios para visar esta colocaci?n',mtError,[mbcancel],0);
              Exit;
           end;
        end;
end;

procedure TfrmVisarDesembolsoPrueba.FormKeyPress(Sender: TObject; var Key: Char);
begin
        EnterTabs(Key,self);
end;

procedure TfrmVisarDesembolsoPrueba.FormClose(Sender: TObject;
...
procedure TfrmVisarDesembolsoPrueba.FormShow(Sender: TObject);
begin
...
procedure TfrmVisarDesembolsoPrueba.Inicializar;
begin
            if dmGeneral.IBTransaction1.InTransaction then
...
procedure TfrmVisarDesembolsoPrueba.CmdTablaLiqClick(Sender: TObject);
...
procedure TfrmVisarDesembolsoPrueba.CmdGarantiasClick(Sender: TObject);
...
procedure TfrmVisarDesembolsoPrueba.CmdCerrarClick(Sender: TObject);
begin
        Close;

end;

procedure TfrmVisarDesembolsoPrueba.CmdVisarClick(Sender: TObject);
var
ColocacionDes:string;
begin
        if MessageDlg('Seguro de Visar Esta Colocaci?n?',mtConfirmation,[mbYes,mbNo],0) = mrYes then
        begin
         CmdVisar.Enabled := False;
         CmdAnular.Enabled := False;
        ...
        { if AplicarDescuentos and AbonarAhorros and AbonarAportes then
          begin
            ...
            AbonarCuotas;// realiza los abonos de las cuotas de los cr?ditos
            ...
            begin
              try
               ...
               MessageDlg('No se Pudo Actualizar la Autorizaci?n del Credivida' + #13 +
          ...
          begin
            ...
            MessageDlg('No Se Pudo Visar la Colocaci?n' + #13 + #10 +
...
function TfrmVisarDesembolsoPrueba.AplicarDescuentos:Boolean;
var
  ...
  begin
    ...
    with IBAbonar do
     try
       ...
       ParamByName('DESCRIPCION').AsString := 'Desembolso de Cr?dito Numero' + colocacion + 'Cuenta Aportes No.' + IntToStr(vCuentaAportes);
...
procedure TfrmVisarDesembolsoPrueba.BuscarDatos;
...
begin
        ...
        with IBAhorros do
         begin
           ...
           IBHonorarios.SQL.Add('select sum(VALOR) as VALOR from "col$pagohonorariostmp" where ID_SOLICITUD = :ID_SOLICITUD');
...
function TfrmVisarDesembolsoPrueba.AbonarAportes:Boolean;
begin
...
function TfrmVisarDesembolsoPrueba.AbonarAhorros:Boolean;
begin
...
procedure TfrmVisarDesembolsoPrueba.CambiarEstadoColocacion;
...
procedure TfrmVisarDesembolsoPrueba.CmdAnularClick(Sender: TObject);
begin
        if MessageDlg('Seguro de Reversar Esta Colocaci?n?',mtConfirmation,[mbYes,mbNo],0) = mrYes then
          begin
            AplicarDescuentos;
            ReversarColocacion;
            elimina_temp;
          ...
          begin
            dmColocacion.IBDScolocacion.Transaction.Rollback;
            MessageDlg('No Se Pudo Reversar la Colocaci?n' + #13 + #10 +
                       'Por Favor Verifique',mtError,[mbOK],0);
          end;
end;

procedure TfrmVisarDesembolsoPrueba.ReversarColocacion;
...
begin
        with IBQuery do
         begin
           ...
           begin
             Descripcion := InputBox('Reversi?n Desembolso','Concepto por el Cual se Reversa','');
           end;
...
function TfrmVisarDesembolsoPrueba.DescontarAhorros: boolean;
begin
    DigitoC := DigitoControl(2,formatcurr('0000000',vCuentaAportes));
          with IBQuery1 do
           begin
...
procedure TfrmVisarDesembolsoPrueba.AbonarCuotas;
begin
...
procedure TfrmVisarDesembolsoPrueba.SpeedButton1Click(Sender: TObject);
var     FileName :string;
begin
...
procedure TfrmVisarDesembolsoPrueba.elimina_temp;
begin
          with IBQuery do
          begin
            Close;
            ...
            ExecSQL;
            SQL.Clear;
            SQL.Add('delete from "col$pagohonorariostmp"');
...
procedure TfrmVisarDesembolsoPrueba.Verifica_credito;
var     saldo :Currency;
...
procedure TfrmVisarDesembolsoPrueba.frReport1GetValue(const ParName: String;
  var ParValue: Variant);
begin
...
procedure TfrmVisarDesembolsoPrueba.imprimir_reporte(cadena: string);
var
...
procedure TfrmVisarDesembolsoPrueba.equivida;
...
function TfrmVisarDesembolsoPrueba.con_equivida: string;
...
begin
        ...
        begin
          ...
          Transaction.StartTransaction;
          SQL.Clear;
          SQL.Add('select max(CONSECUTIVO) + 1 as CONSECUTIVO from');
          SQL.Add('"gen$con_seguro"');
          ExecQuery;
          consecutivo := FieldByName('CONSECUTIVO').AsInteger;
          if consecutivo = 623 then
             consecutivo := consecutivo + 1;
          Close;
          SQL.Clear;
          SQL.Add('insert into "gen$con_seguro" values(:ID_AGENCIA,:CONSECUTIVO)');
          ParamByName('ID_AGENCIA').AsInteger := Agencia;
...
procedure TfrmVisarDesembolsoPrueba.VerificaCaptacion;
var
...
procedure TfrmVisarDesembolsoPrueba.AbonaHonorarios;
begin
        with IBSQL1 do begin
          if Transaction.InTransaction then
             Transaction.Commit;
          ...
          Close;
          SQL.Clear;
          SQL.Add('select * from "col$pagohonorariostmp" where ID_COLOCACION_DES = :ID_COLOCACION_DES');
          ...
          while not IBSQL1.Eof do begin
            ...
            IBHonorarios.ParamByName('DOCUMENTO_MOVIMIENTO').AsString := FormatCurr('00000000',IBSQL1.FieldByName('ID_COMPROBANTE').AsInteger);
            ...
            IBHonorarios.ParamByName('VALOR_DEBITO').AsCurrency := IBSQL1.FieldByName('VALOR').AsCurrency;
...
procedure TfrmVisarDesembolsoPrueba.AutorizacionCredivida;
...
begin
    ...
    ano := IntToStr(YearOf(fFechaActual)+ 1);
        ...
        begin
          Close;
          SQL.Clear;
          SQL.Add('insert into "col$credivida" values (:NUMERO_CUENTA,:DG_CUENTA,:FECHA_VENCIMIENTO,0)');
          ParamByName('NUMERO_CUENTA').AsInteger := vCuentaAportes;
