...
uses
  ...
  IBDatabase, strutils, Fr_Dbset, DBClient;

type
  TfrmVisarDesembolsoTmp = class(TForm)
    Panel1: TPanel;
    GroupBox1: TGroupBox;
    Label18: TLabel;
    Label1: TLabel;
  ...
  // VARIABLES MANEJO DE DESEMBOLSO
    ...
    function AbonarAportes(_iTipoCAptacion:Integer):Boolean;
...
var
  frmVisarDesembolsoTmp: TfrmVisarDesembolsoTmp;
  colocacion : string;
  vcolocacion : string;
...
procedure TfrmVisarDesembolsoTmp.EdNumeroColocacionExit(Sender: TObject);
...
procedure TfrmVisarDesembolsoTmp.FormKeyPress(Sender: TObject; var Key: Char);
begin
        EnterTabs(Key,self);
end;

procedure TfrmVisarDesembolsoTmp.FormClose(Sender: TObject;
...
procedure TfrmVisarDesembolsoTmp.FormShow(Sender: TObject);
begin
...
procedure TfrmVisarDesembolsoTmp.Inicializar;
begin
            if dmGeneral.IBTransaction1.InTransaction then
...
procedure TfrmVisarDesembolsoTmp.CmdTablaLiqClick(Sender: TObject);
...
procedure TfrmVisarDesembolsoTmp.CmdGarantiasClick(Sender: TObject);
...
procedure TfrmVisarDesembolsoTmp.CmdCerrarClick(Sender: TObject);
begin
        Close;
end;

procedure TfrmVisarDesembolsoTmp.CmdVisarClick(Sender: TObject);
var
...
begin
        ...
        begin
         ...
         if AplicarDescuentos and AbonarAhorros and AbonarAportes(7) then
          begin
            DescontarAhorros;
...
function TfrmVisarDesembolsoTmp.AplicarDescuentos:Boolean;
var
...
procedure TfrmVisarDesembolsoTmp.BuscarDatos;
...
function TfrmVisarDesembolsoTmp.AbonarAportes(_iTipoCAptacion:Integer):Boolean;
begin
    if ValorAportes > 0 then
    begin
      DigitoC := DigitoControl(_iTipoCaptacion,formatcurr('0000000',vCuentaAportes));
      //validar existencia de cuenta del pasivo
      CrearProducto(vCuentaAportes,_iTipoCAptacion,StrToInt(DigitoC),vIdIdentificacion,vIdPersona);
      Result := True;
      with IBQuery1 do
      begin
        SQL.Clear;
...
function TfrmVisarDesembolsoTmp.AbonarAhorros:Boolean;
begin
...
procedure TfrmVisarDesembolsoTmp.CambiarEstadoColocacion;
...
procedure TfrmVisarDesembolsoTmp.CmdAnularClick(Sender: TObject);
begin
...
procedure TfrmVisarDesembolsoTmp.ReversarColocacion;
...
function TfrmVisarDesembolsoTmp.DescontarAhorros: boolean;
begin
    DigitoC := DigitoControl(2,formatcurr('0000000',vCuentaAportes));
          with IBQuery1 do
           begin
...
procedure TfrmVisarDesembolsoTmp.AbonarCuotas;
...
procedure TfrmVisarDesembolsoTmp.SpeedButton1Click(Sender: TObject);
var     FileName :string;
begin
...
procedure TfrmVisarDesembolsoTmp.elimina_temp;
begin
          with IBQuery do
          begin
            Close;
...
procedure TfrmVisarDesembolsoTmp.Verifica_credito;
var     saldo :Currency;
...
procedure TfrmVisarDesembolsoTmp.frReport1GetValue(const ParName: String;
  var ParValue: Variant);
begin
...
procedure TfrmVisarDesembolsoTmp.imprimir_reporte(cadena: string);
begin
...
procedure TfrmVisarDesembolsoTmp.equivida;
...
function TfrmVisarDesembolsoTmp.con_equivida: string;
...
procedure TfrmVisarDesembolsoTmp.VerificaCaptacion;
var
...
procedure TfrmVisarDesembolsoTmp.AbonaHonorarios;
begin
        with IBSQL1 do begin
          if Transaction.InTransaction then
             Transaction.Commit;
...
procedure TfrmVisarDesembolsoTmp.AutorizacionCredivida;
...
procedure TfrmVisarDesembolsoTmp.InfDesembolso;
begin
        with IBQuery do
        begin
          Close;
