...
type
  TFrmDesembolsoTmp = class(TForm)
    ...
    CDBancoldexFECHA_ACTIVOS: TDateField;
    ...
    IBQuery3: TIBQuery;
    PCuenta: TPanel;
    DsTipocuanta: TDataSource;
    IbTipocuenta: TIBQuery;
    Label10: TLabel;
    Label11: TLabel;
...
var
  FrmDesembolsoTmp: TFrmDesembolsoTmp;
  colocacion:string;
  vcolocacion : string;
...
procedure TFrmDesembolsoTmp.FormCreate(Sender: TObject);
begin
...
procedure TFrmDesembolsoTmp.CBtiposidEnter(Sender: TObject);
begin
        CBtiposid.DropDown
end;

procedure TFrmDesembolsoTmp.CBtiposidKeyPress(Sender: TObject; var Key: Char);
begin
        EnterTabs(Key,Self)
end;

procedure TFrmDesembolsoTmp.CBtiposidExit(Sender: TObject);
begin
        try
...
procedure TFrmDesembolsoTmp.EdNumeroIdentificacionExit(Sender: TObject);
begin
             ...
             if RecordCount = 1 then
               begin
                 ...
                 begin
                   ...
                   if RecordCount < 1 then
                    begin
                      MessageDlg('No Posee Solicitudes de Cr?dito Aprobadas',mtError,[mbok],0);
        ...
        With IBSQL1 do
         begin
           ...
           if (YearsBetween(Now,FieldByName('FECHA_NACIMIENTO').AsDate)) > 70 then
              ...
              es_mayor := False;
           Chedad.Hint := IntToStr(YearsBetween(Now,FieldByName('FECHA_NACIMIENTO').AsDate)) + ' A?os';
...
procedure TFrmDesembolsoTmp.EdNumeroIdentificacionKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self)
end;

procedure TFrmDesembolsoTmp.DBsolicitudExit(Sender: TObject);
begin
        try
...
procedure TFrmDesembolsoTmp.DBsolicitudKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self)
end;

procedure TFrmDesembolsoTmp.BitBuscarClick(Sender: TObject);
var     nombre,apellido1,apellido2 :string;
        valor_desmbolso :Currency;
        desc_garantia :string;
        vEstadoSolicitud :Smallint;
        vFechaVencimiento :TDate;
        vDias :Integer;                    
        vFechaConcepto :TDate;
        vDiasConcepto :Integer;
        vDiasVigenciaSol :Integer;
        vTasaMaxClasificacion: Double;
        vFechaGracia:TDate;
        vFechaMaximo :TDate;
begin
        ...
        begin
          ...
          begin
            ...
            ExecQuery;
            vDiasVigenciaSol := FieldByName('VALOR_MINIMO').AsInteger;
            Close;
            Close;
            SQL.Clear;
            ...
            ExecQuery;
            vFechaConcepto := FieldByName('FECHA_CONCEPTO').AsDateTime;
            vEstadoSolicitud :=  FieldByName('ESTADO').AsInteger;
            ...
            //validacion de la vigencia de la solicitud
            // validacion fecha 25 de abril
            vDiasConcepto := DaysBetween(fFechaActual,vFechaConcepto);
            if vFechaConcepto <= vFechaMaximo then
               vDiasVigenciaSol := vDiasVigenciaSol + 30;
            if  vDiasConcepto >= vDiasVigenciaSol then
            begin
                Close;
                SQL.Clear;
                SQL.Add('SELECT ');
                ...
                SQL.Add('WHERE');
                SQL.Add('("col$registrosesion".ID_SOLICITUD = :ID_SOLICITUD) AND ');
                SQL.Add('("col$registrosesion".ESTADO = 16) AND ');
                SQL.Add('("col$registrosesion".ID_AGENCIA = :ID_AGENCIA)');
                ParamByName('ID_SOLICITUD').AsString := vIdSolicitud;
                ParamByName('ID_AGENCIA').AsInteger := Agencia;
                ExecQuery;
                if RecordCount <= 0 then
                begin
                  MessageDlg('La Solicitud debe ser Anulada Por Vencimiento de Tiempo...',mtWarning,[mbok],0);
                  inicializar;
                  Exit;
                end;
            end;
            ...
            Close;
            if vLinea = 6 then// valida linea cdta normal
            begin
              IBcdat.Close;
              IBcdat.SQL.Clear;
              IBcdat.SQL.Add('SELECT ');
              IBcdat.SQL.Add('"col$cdatsolicitud".NUMERO_CDAT');
              IBcdat.SQL.Add('FROM');
              IBcdat.SQL.Add('"col$cdatsolicitud"');
              IBcdat.SQL.Add('WHERE');
              IBcdat.SQL.Add('("col$cdatsolicitud".ID_SOLICITUD = :ID_SOLICITUD)');
              IBcdat.ParamByName('ID_SOLICITUD').AsString := DBsolicitud.KeyValue;
              IBcdat.ExecQuery;
              if IBcdat.RecordCount = 0 then
                 MessageDlg('Favor Verifique los Cdats o el valor del Interes, Cdats no encontrados',mtWarning,[mbok],0);
              Close;
              SQL.Clear;
              SQL.Add('SELECT');
              ...
              ParamByName('NUMERO').AsInteger := IBcdat.FieldByName('NUMERO_CDAT').AsInteger;
              ExecQuery;
              if RecordCount <> 0 then
              begin
                if FieldByName('ID_ESTADO').AsInteger = 1 then
                   vFechaVencimiento := FieldByName('FECHA_VENCIMIENTO').AsDate
                  else if FieldByName('ID_ESTADO').AsInteger = 6 then
                     vFechaVencimiento := FieldByName('FECHA_VENCIMIENTO_PRORROGA').AsDateTime
                    else
                    begin
                       MessageDlg('Verifique el CDAT no se Encuentra Vigente',mtInformation,[mbok],0);
                       cmdnuevo.Click;
                       Exit;
                    end;
                    Vdias := DiasEntre(fFechaActual,vFechaVencimiento);
                  if vDias < vPlazo then
                  begin
                     MessageDlg('El Vencimiento del CDAT es Menor que el Vencimiento del Cr?dito' + #13 + ' Plazo M?ximo Pemitido por el CDAT ' + IntToStr(vDias) + ' Dias a Fecha Hoy',mtInformation,[mbok],0);
                     cmdnuevo.Click;
                     Exit;
                  end;
                 vTasaEfectiva := FieldByName('TASA_EFECTIVA').AsFloat * seguro(7);
              end; //fin verificacion numero de cdat
            end; // fin validacion de la linea cdat
            if vLinea = 12 then// valida linea cdta ORDINARIA
            begin
              IBcdat.Close;
              IBcdat.SQL.Clear;
              IBcdat.SQL.Add('SELECT ');
              ...
              begin
                ...
                SQL.Add('WHERE');
                SQL.Add('("cap$maestro".ID_TIPO_CAPTACION = 6) AND');
                SQL.Add('("cap$maestro".NUMERO_CUENTA = :NUMERO) AND');
                SQL.Add('("cap$maestro".ID_ESTADO IN (1,6))');
                ParamByName('NUMERO').AsInteger := IBcdat.FieldByName('NUMERO_CDAT').AsInteger;
                ExecQuery;
                if RecordCount = 0 then
                begin
                   MessageDlg('Verifique el CDAT No. '+ IBcdat.FieldByName('NUMERO_CDAT').AsString + ' no se Encuentra Vigente',mtInformation,[mbok],0);
                   cmdnuevo.Click;
                   Exit;
                end;
                IBcdat.Next;
                 ...
                 //vTasaEfectiva := FieldByName('TASA_EFECTIVA').AsFloat * seguro(7);
            end; // fin validacion de la linea cdat ORDINARIA

            if vLinea = 13 then begin //Verifica Linea Mercado Publico
              Close;
              SQL.Clear;
              ...
              EDperiodogracia.Text := IntToStr(vPeriodoGracia);
              Close;
            end; // Fin Verifica Linea Mercado

            // Inicio Verificacion de Tasa por Clasificacion
            vTasaMaxClasificacion := BuscoTasaEfectivaMaxima(fFechaActual,vClasificacion,'A');
            if vTasaMaxClasificacion < vTasaEfectiva then
              vTasaEfectiva := vTasaMaxClasificacion;
            // Fin Verificacion Tasa por Clasificacion
            ...
            if RecordCount > 0 then begin
              ...
              begin
                 ...
                 if IBSQL11.Transaction.InTransaction then
                   IBSQL11.Transaction.Rollback;
                 IBSQL11.Transaction.StartTransaction;
                 if FieldByName('DESTINO1').AsInteger > 0 then begin
                   IBSQL11.Close;
                   IBSQL11.ParamByName('CODIGO').AsInteger := FieldByName('DESTINO1').AsInteger;
                   IBSQL11.ExecQuery;
                   CDBancoldex.FieldValues['DESTINO1'] := IBSQL11.FieldByName('NOMBRE').AsString;
                   CDBancoldex.FieldValues['MONTO1'] := FieldByName('MONTO1').AsCurrency;
                 end;
                 if FieldByName('DESTINO2').AsInteger > 0 then begin
                   IBSQL11.Close;
                   IBSQL11.ParamByName('CODIGO').AsInteger := FieldByName('DESTINO2').AsInteger;
                   IBSQL11.ExecQuery;
                   CDBancoldex.FieldValues['DESTINO2'] := IBSQL11.FieldByName('NOMBRE').AsString;
                   CDBancoldex.FieldValues['MONTO2'] := FieldByName('MONTO2').AsCurrency;
                 end;
                 if FieldByName('DESTINO3').AsInteger > 0 then begin
                   IBSQL11.Close;
                   IBSQL11.ParamByName('CODIGO').AsInteger := FieldByName('DESTINO3').AsInteger;
                   IBSQL11.ExecQuery;
                   CDBancoldex.FieldValues['DESTINO3'] := IBSQL11.FieldByName('NOMBRE').AsString;
                   CDBancoldex.FieldValues['MONTO3'] := FieldByName('MONTO3').AsCurrency;
                 end;
                 if FieldByName('GARANTIA1').AsInteger > 0 then begin
                   IBSQL12.Close;
                   IBSQL12.ParamByName('CODIGO').AsInteger := FieldByName('GARANTIA1').AsInteger;
                   IBSQL12.ExecQuery;
                   CDBancoldex.FieldValues['GARANTIA1'] := IBSQL12.FieldByName('NOMBRE').AsString;
                   CDBancoldex.FieldValues['VGARANTIA1'] := FieldByName('VALORGAR1').AsCurrency;
                 end;
                 if FieldByName('GARANTIA2').AsInteger > 0 then begin
                   IBSQL12.Close;
                   IBSQL12.ParamByName('CODIGO').AsInteger := FieldByName('GARANTIA2').AsInteger;
                   IBSQL12.ExecQuery;
                   CDBancoldex.FieldValues['GARANTIA2'] := IBSQL12.FieldByName('NOMBRE').AsString;
                   CDBancoldex.FieldValues['VGARANTIA2'] := FieldByName('VALORGAR2').AsCurrency;
                 end;
                 if FieldByName('GARANTIA3').AsInteger > 0 then begin
                   IBSQL12.Close;
                   IBSQL12.ParamByName('CODIGO').AsInteger := FieldByName('GARANTIA3').AsInteger;
                   IBSQL12.ExecQuery;
                   CDBancoldex.FieldValues['GARANTIA3'] := IBSQL12.FieldByName('NOMBRE').AsString;
              ...
              if LineaBancoldex <> '' then begin
                Close;
                SQL.Clear;
                SQL.Add('select PUNTOS_ADIC, SALDO from "col$creditosbancoldex" where linea = :Linea and id_obligacion = :Obligacion');
                ParamByName('LINEA').AsString := LineaBancoldex;
                ParamByName('OBLIGACION').AsString := NoBancoldex;
                ExecQuery;
                if vDesembolso > FieldByName('SALDO').AsCurrency then begin
                  MessageDlg('No se Puede Desembolsar Porque El Valor del Desembolso Supera el Saldo Bancoldex',mtWarning,[mbok],0);
                  inicializar;
                  Exit;
                end;
//                vPuntosAdicionales := FieldByName('PUNTOS_ADIC').AsFloat;
//                vvalorpuntos := vpuntosadicionales;
//                EDpuntos.Text := FormatCurr('#0.00',vPuntosAdicionales);
              end;
            end;
...
function TFrmDesembolsoTmp.descripcion(opcion, tipo: integer): string;
begin
        with IBSQL2 do
        begin
          Close;
...
procedure TFrmDesembolsoTmp.CmdCerrarClick(Sender: TObject);
begin
        Close;
...
procedure TFrmDesembolsoTmp.inicializar;
begin
    if dmGeneral.IBTransaction1.InTransaction then
       dmGeneral.IBTransaction1.Rollback;
    dmGeneral.IBTransaction1.StartTransaction;
...
procedure TFrmDesembolsoTmp.CmdNuevoClick(Sender: TObject);
begin
        inicializar
end;

procedure TFrmDesembolsoTmp.CmdGrabarClick(Sender: TObject);
begin
        ...
        begin
           MessageDlg('Los Descuentos Exceden el Valor del Desembolso',mtWarning,[mbok],0);
           Exit;
        end;
        ...
        begin
           ...
           try
             ...
             MessageDlg('Desembolso Grabado con Exito, Pero no se Guardo Informacion Extra del Credito Favor Informe a Sistemas',mtWarning,[mbok],0);
           end;
//           Visado;
        ...
        begin
           elimina_temp;
           MessageDlg('No Se Pudo Crear la Nueva Colocaci?n' + #13 + #10 +
                      'Por Favor Verifique',mtError,[mbOK],0);
        end;
end;

procedure TFrmDesembolsoTmp.FormClose(Sender: TObject;
...
procedure TFrmDesembolsoTmp.EdTipoTasaEnter(Sender: TObject);
begin
        if vTipoTasa = 'F' then
         EdTipoTasa.ItemIndex := 0
        else
...
procedure TFrmDesembolsoTmp.FormShow(Sender: TObject);
begin
        try
          Inicializar;
        finally
...
procedure TFrmDesembolsoTmp.BitBuscarKeyPress(Sender: TObject; var Key: Char);
begin
        EnterTabs(Key,Self);
end;

procedure TFrmDesembolsoTmp.EDperiodograciaKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self)
end;

procedure TFrmDesembolsoTmp.EDperiodograciaEnter(Sender: TObject);
begin
        EdPeriodoGracia.Text := FormatCurr('#0',vPeriodoGracia);
end;

procedure TFrmDesembolsoTmp.EDperiodograciaExit(Sender: TObject);
begin
        try
...
procedure TFrmDesembolsoTmp.EdtipotasaExit(Sender: TObject);
begin
...
procedure TFrmDesembolsoTmp.CBtasavariableClick(Sender: TObject);
begin
        vTasaEfectiva := dmColocacion.IBDStasavariable.FieldValues['VALOR_ACTUAL_TASA'];
        EdTasaEfectiva.Text := FormatCurr('#0.00%',vTasaEfectiva);
...
procedure TFrmDesembolsoTmp.CBtasavariableKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self)
end;

procedure TFrmDesembolsoTmp.CBtasavariableExit(Sender: TObject);
begin
...
procedure TFrmDesembolsoTmp.EdtipotasaKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self)
end;

procedure TFrmDesembolsoTmp.EDtasaefectivaExit(Sender: TObject);
var Plazo:Integer;
begin
...
///*****************fin tipo de cuota
        ...
        if vLinea = 13 then
          with IBQuery3 do
           begin
             Transaction := dmColocacion.IBQuery.Transaction;
             Close;
             SQL.Clear;
             SQL.Add('select VALOR_MINIMO from "gen$minimos" WHERE ID_MINIMO = 33');
             Open;
...
{          if vLinea = 13 then
        ...
        if vLinea = 13 then
          vFechaVencimiento := CalculoFecha(vFechaVencimiento,vPlazo)
        else
...
procedure TFrmDesembolsoTmp.BTadmisibleClick(Sender: TObject);
begin
        LimpiaInformacion;
        DBGridReal.Visible := True;
        GrupoInformacion.Caption := 'Informaci?n Garantias Real';
end;

procedure TFrmDesembolsoTmp.BTnoadmisibleClick(Sender: TObject);
begin
        LimpiaInformacion;
        DBGridPersonal.Visible := True;
        GrupoInformacion.Caption := 'Informaci?n Garantias Personales';
end;

procedure TFrmDesembolsoTmp.LimpiaInformacion;
begin
        ...
        DBGridBancoldex.Visible := False;
        Panel1.Visible := False;
        PCuenta.Visible := False;
        Grupoinformacion.Caption := '';
end;

procedure TFrmDesembolsoTmp.colocaciones(colocacion: string);
...
procedure TFrmDesembolsoTmp.ColocacionesJudCas(Colocacion:string);
var    valor_tasa :Currency;
begin
  with IBSQL2 do
   begin
...
procedure TFrmDesembolsoTmp.barrido_cuotas(id_persona,id_colocacion:string;id_identificacion:Integer);
...
procedure TFrmDesembolsoTmp.BTtotalClick(Sender: TObject);
begin
        limpiainformacion;
        DBGridDescParcial.Visible := True;
        Grupoinformacion.Caption := 'Informaci?n Descuentos';
end;

procedure TFrmDesembolsoTmp.BTparcialClick(Sender: TObject);
begin
        LimpiaInformacion;
        CDcuotas.IndexFieldNames := 'tipo';
        DBGridCuotas.Visible := True;
        GrupoInformacion.Caption := 'Informaci?n Cr?ditos Atrasados';
end;


procedure TFrmDesembolsoTmp.ConsultaColocacin1Click(Sender: TObject);
begin
           control_consulta := False;
           frmConsultaColocacion := TfrmConsultaColocacion.Create(Self);
//           agencia := CDcu otasid_agencia.Value;
...
procedure TFrmDesembolsoTmp.activa_colocacion;
begin
        with dmColocacion do
        begin
                IBDStiposid.Active := true;
...
procedure TFrmDesembolsoTmp.ConsultaColocacion1Click(Sender: TObject);
begin
           control_consulta := False;
           frmConsultaColocacion := TfrmConsultaColocacion.Create(Self);
...
procedure TFrmDesembolsoTmp.busca_fianzas(id_persona: string;
  id_identificacion: integer);
begin
        ...
        begin
          ...
          while not Eof do
          begin
            if vfianza(FieldByName('ID_COLOCACION').AsString) = False  then
...
procedure TFrmDesembolsoTmp.EDtasaefectivaKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self)
end;

function TFrmDesembolsoTmp.seguro(id_seguro: integer): currency;
begin
        with DmSolicitud.IBSolicitud1 do
        begin
          Close;
...
procedure TFrmDesembolsoTmp.EDvalorcolocacionKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self)
end;

procedure TFrmDesembolsoTmp.EDtasanomKeyPress(Sender: TObject; var Key: Char);
begin
        EnterTabs(Key,Self)
end;

procedure TFrmDesembolsoTmp.BTmostrarClick(Sender: TObject);
begin
        LimpiaInformacion;
        Panel1.Visible := True;
end;

procedure TFrmDesembolsoTmp.liquidatotal;
...
procedure TFrmDesembolsoTmp.BTliquidaClick(Sender: TObject);
begin
     ...
     begin
       ...
       frmProgreso.Titulo := 'Proceso de Liquidaci?n de Cuotas';
       frmProgreso.Min := 0;
...
procedure TFrmDesembolsoTmp.CmdInformeClick(Sender: TObject);
var
PuntosAdic : Double;
...
//VidColocacion := '20050003598';
        ...
        begin
             ...
             SQL.Clear;
             SQL.Add('SELECT ');
             SQL.Add('SUM("col$pagohonorariostmp".VALOR) AS VALOR');
             SQL.Add('FROM');
             SQL.Add('"col$pagohonorariostmp"');
             SQL.Add('WHERE');
             SQL.Add('("col$pagohonorariostmp".ID_SOLICITUD = :ID_SOLICITUD)');
             ParamByName('ID_SOLICITUD').AsString := DBsolicitud.KeyValue;
             Open;
             if RecordCount > 0 then
               vhonorarios := FieldByName('VALOR').AsCurrency;
        end;
          vhonorarios := 0;
        end;
        ...
        valor_total := recupera_saldo(8,0,'C') - vhonorarios;
...
procedure TFrmDesembolsoTmp.frReport1GetValue(const ParName: String;
  var ParValue: Variant);
...
begin
        ...
        begin
           valor_neto := valor_neto + valor_otros;
           valor_otros := 0;
        end;
...
//        if ParName = 'otros' then
//           ParValue :=


end;

procedure TFrmDesembolsoTmp.imprimir_reporte(cadena: string);
begin
...
procedure TFrmDesembolsoTmp.liquidaparcial;
var CuotasLiq:TCuotasLiq;
    AF:PCuotasLiq;
    AR:PCuotasLiq;
    EnBilletes:Currency;
...
procedure TFrmDesembolsoTmp.grabar_extracto;
var
...
procedure TFrmDesembolsoTmp.tablaliquidacion;
begin
        with IBQuery1 do
         begin
          SQL.Clear;
          ...
          try
            ...
            Transaction.Rollback;
            MessageDlg('Error al al Realizar Transacci?n',mtInformation,[mbOK],0);
            raise;
          end;
         end;
end;

procedure TFrmDesembolsoTmp.DBGridCuotasDblClick(Sender: TObject);
begin
        CDcuotas.Delete
end;

procedure TFrmDesembolsoTmp.BTcuotasClick(Sender: TObject);
begin
         with IBGrupo1 do
          begin
          if Transaction.InTransaction then
...
procedure TFrmDesembolsoTmp.AgregarCrdito1Click(Sender: TObject);
var       numero_colocacion :string;
begin
        numero_colocacion := InputBox('Colocaci?n Numero','Digite N?mero de Colocacion','');
     ...
     begin
        ...
        begin
          ...
          ExecQuery;
          if RecordCount = 0 then
             MessageDlg('Colocaci?n no Encontrada',mtInformation,[mbok],0)
          else  if FieldByName('ID_ESTADO_COLOCACION').AsInteger <> 0 then
             MessageDlg('La colocaci?n No :'+FieldByName('ID_COLOCACION').AsString+ ' ' +'No se Puede Descontar' + #13 + 'Por Estar en Estado '+FieldByName('DESCRIPCION_ESTADO_COLOCACION').AsString,mtWarning,[mbok],0)
          else
          begin
...
procedure TFrmDesembolsoTmp.grabar_garantias;
begin
        ...
        begin
           ...
           begin
             ...
             begin
               ...
               MessageDlg('Error insertando en col$garantias',mtError,[mbok],0);
               end;
               CDgarantias.Next;
             end;
           end;
           ...
           begin
             ...
             begin
                ...
                SQL.Add(':"CODIGO_ASEGURADORA",:"ES_HIPOTECA")');
                ParamByName('ID_AGENCIA').AsInteger := Agencia;
                ParamByName('ID_COLOCACION').AsString := vIdColocacion;
                ...
                ParamByName('ES_HIPOTECA').AsBoolean := True;
                try
                ExecSQL;
                except
                MessageDlg('Error insertando en col$garantiasreal',mtError,[mbok],0);
                end;
                CDgarantiareal.Next;
             end;
           end;
...
procedure TFrmDesembolsoTmp.contabilizar;
var i:Integer;
    vVecesAportes,
    vVecesAhorros,
    vPorcCredito,
    ...
    AF:PFechasLiq;
    J:Integer;
    ValorSolicitud :Currency;
    ValorIva :Currency;
    vSeDescuenta :Boolean;
...
begin
  try
        ...
        begin
          ...
          Open;
          vVecesMin := FieldByName('VALOR_MINIMO').AsCurrency;
          Close;
          SQL.Clear;
          SQL.Add('select VALOR_MINIMO from "gen$minimos" where ID_MINIMO = 32');
          Open;
          vSalarioMin := FieldByName('VALOR_MINIMO').AsCurrency;
          if (vSalarioMin * vVecesMin) <= EDvalorcolocacion.Value then
             vSeDescuenta := True
          else
            vSeDescuenta := False;
        end;
        ...
        begin
          ValorSolicitud := SimpleRoundTo((seguro(8)/((seguro(22) / 100) + 1)),0);
          ValorIva := seguro(8) - ValorSolicitud;
          with dmColocacion.IBQuery do
          begin
            SQL.Clear;
        ...
        begin
            ...
            begin
              if DiasEntre(EdFechaPago.Date,dmColocacion.IBQuery.FieldbyName('FECHA_A_PAGAR').AsDateTime) <= 360 then
        ...
        if interes_anticipado then begin
          with dmColocacion.IBSQLcodigos do
           begin
             ...
             if RecordCount = 0 then
              begin
                MessageDlg('No Existen C?digos contables para liquidar esta Colocaci?n',mtError,[mbOk],0);
                Close;
                Exit;
              end;
          end;
            ...
            if vDescuento > 0 then begin
              ...
              // extracto interes por fecha de pago
                  ...
                  begin
                    ...
                    ParamByName('ABONO_ANTICIPADO').AsCurrency := vDescuento;
                    ...
                    ParamByName('VALOR_DEBITO').AsCurrency := vDescuento;
                    ParamByName('VALOR_CREDITO').AsCurrency := 0;
                    ExecQuery;
                    Close;
                end;
              // fin extracto interes por fecha de pago
                 ...
                 if vLinea  = 13 then
                  begin
                    with dmColocacion.IBQuery do begin
                     ...
                     Close;
                     SQL.Clear;
                     SQL.Add('select first 1 FECHA_A_PAGAR from "col$tablaliquidacion" where ID_COLOCACION = :COLOCACION ');
                     SQL.Add(' and INTERES_A_PAGAR > 0');
                     ParamByName('COLOCACION').AsString := VidColocacion;
                     Open;
                     if RecordCount > 0 then
                       vDiasIntAnt := DiasEntre(vFechaGracia,FieldByName('FECHA_A_PAGAR').AsDateTime)
                     else
                       vDiasIntAnt := 0;
                    end; // Fin de With IBQuery
                    vInteresAnt := SimpleRoundTo(vDesembolso*(vTasaNominal/100)*vDiasIntAnt/360,0);
                  end
                 else
...
{Busco Codigo Capitalizaci?n y Aplico Valor Capitalizacion}
///************************///
           SQL.Clear;
           SQL.Add('select CODIGO from COL$CODIGOSPUCBASICOS ');
           SQL.Add('where ID_CODIGOPUCBASICO = 63');//3
           Open;
           vCodigoPuc := FieldByName('CODIGO').AsString;
           Close;
           dmColocacion.IBDScontable.Append;
...
{fin Busco Codigo Capitalizaci?n y Aplico Valor Capitalizacion}
        end;
  ...
  Except
    MessageDlg('Error insertando en contabilizando',mtError,[mbok],0);
    dmColocacion.IBDScolocacion.Transaction.Rollback;
  end;

end;

function TFrmDesembolsoTmp.BuscarAportes(TipoId: Integer;
  DocumentoId: string): Currency;
var Saldo,MinAportes:Currency;
...
begin
   with IBSQL2 do begin
     ...
     SQL.Clear;
     SQL.Add('select SALDO_DISPONIBLE from SALDO_DISPONIBLE_JUD (:AG,:TP,:CTA,:DGT,:ANO,:FECHA1,:FECHA2)');
     ParamByName('AG').AsInteger := Agencia;
     ParamByName('TP').AsInteger := 1;
     ...
     Close;
     SQL.Clear;
     SQL.Add('SELECT SALDO_MINIMO from "cap$tipocaptacion" where ID_TIPO_CAPTACION = 1');
     ExecQuery;
     MinAportes := FieldByName('SALDO_MINIMO').AsCurrency;
   end;
//     Saldo := Saldo - MinAportes;
     Result := Saldo;
end;

function TFrmDesembolsoTmp.VerificarAportesCreditosAnt(
  TipoIdentificacion: Integer; NumeroIdentificacion: String): Currency;
var
Saldo : Currency;
VecesAportes : Currency;
...
begin
        ...
        begin
          ...
          while not dmColocacion.IBQuery1.Eof do
           begin
             CDcolParcial.Filtered := False;
             CDcolParcial.Filter := 'id_colocacion = ' + FieldByName('ID_COLOCACION').AsString;
             CDcolParcial.Filtered := True;
             if CDcolParcial.RecordCount = 0 then
             begin
               Saldo := FieldByName('SALDO').AsCurrency;
               VecesAportes := FieldByName('RECIPROCIDAD').AsCurrency;
               if VecesAportes = 0 then
                  VecesAportes := FieldByName('VECES_APORTES').AsCurrency;
...
function TFrmDesembolsoTmp.CrearColocacion: Boolean;
var Consec:String;
vFechaGracia :TDate;
NFechaK,NFechaI:TDate;
begin
{ Validaciones}
            ...
            begin
               messagedlg('No diligencio el valor de la colocaci?n',mtError,[mbOk],0);
               Result := false;
               exit;
            end;
            if vPlazo < 1 then
            begin
               messagedlg('No diligencio el plazo de la colocaci?n',mtError,[mbOk],0);
               Result := false;
               exit;
            end;
            ...
            begin
               messagedlg('El valor de la amortizaci?n a capital no es valido',mtError,[mbOk],0);
               Result := false;
               exit;
            end;
            if vAmortizaInteres < 1 then
            begin
               messagedlg('El valor de la amortizaci?n a interes no es valido',mtError,[mbOk],0);
               Result := false;
               exit;
            end;
{fin Validaciones}
              ...
              if GrabarColocacion then
               begin
                 ...
                 begin
                  ...
                  begin
                   SQL.Clear;
                   SQL.Add('select FIRST 1 FECHA_A_PAGAR from "col$tablaliquidacion" where ID_AGENCIA = :ID AND ID_COLOCACION = :COLOCACION');
                   SQL.Add('AND CAPITAL_A_PAGAR > 0  ORDER BY CUOTA_NUMERO ASC');
                   ParamByName('ID').AsInteger := Agencia;
                   ParamByName('COLOCACION').AsString := VidColocacion;
                   Open;
                   NFechaK := FieldByName('FECHA_A_PAGAR').AsDateTime;
                   Close;
                  end;
                  ...
                  begin
                   SQL.Clear;
                   SQL.Add('select FIRST 1 FECHA_A_PAGAR from "col$tablaliquidacion" where ID_AGENCIA = :ID AND ID_COLOCACION = :COLOCACION');
                   SQL.Add('AND INTERES_A_PAGAR > 0  ORDER BY CUOTA_NUMERO ASC');
                   ParamByName('ID').AsInteger := Agencia;
                   ParamByName('COLOCACION').AsString := VidColocacion;
                   Open;
                   NFechaI := FieldByName('FECHA_A_PAGAR').AsDateTime;
                   Close;
                  end;

                  if vCuotaTipo = 'F' then
                   begin
                     NFechaK := CalculoFecha(NFechaK,-vAmortizaCapital);
                     NFechaI := CalculoFecha(NFechaI,-vAmortizaInteres);
                   end;
                   
                  vFechaInteres := NFechaI;

                  with IBQuery3 do
                  begin
                   SQL.Clear;
                   SQL.Add('update "col$colocacion" set VALOR_CUOTA = :CUOTA, FECHA_CAPITAL = :FECHAK, FECHA_INTERES = :FECHAI where ID_AGENCIA = :ID AND ID_COLOCACION = :COLOCACION');
                   ParamByName('ID').AsInteger := Agencia;
                   ParamByName('COLOCACION').AsString := VidColocacion;
                   ParamByName('CUOTA').AsCurrency := vValorCuota;
                   ParamByName('FECHAK').AsDate := NFechaK;
                   ParamByName('FECHAI').AsDate := NFechaI;
                   Open;
                  end; //fin de With
...
function TFrmDesembolsoTmp.GrabarColocacion: boolean;
begin
...
//credito no visado por defecto
                ...
                if esbancoldex then begin
                  SQL.Clear;
                  SQL.Add('update "col$infbancoldex" set ID_COLOCACION = :ID_COLOCACION, TASA_EA = :TASA where ID_SOLICITUD = :ID_SOLICITUD');
                  ParamByName('ID_SOLICITUD').AsString := vIdSolicitud;
                  ParamByName('ID_COLOCACION').AsString := VidColocacion;
                  ParamByName('TASA').AsFloat := vTasaEfectiva;
                  ExecSQL;
                  Close;
                end;
              end;
              ...
              MessageDlg('Error insertando en col$colocacion',mtError,[mbok],0);
              Result := false;
              dmGeneral.IBTransaction1.Rollback;
           end;
end;

procedure TFrmDesembolsoTmp.EdFechaPagoExit(Sender: TObject);
begin
        VfechaPago := EdFechaPago.Date;
        if vlinea = 13 then
           vFechaVencimiento := CalculoFecha(VfechaPago,vPlazo)
        else
...
procedure TFrmDesembolsoTmp.EdFechaPagoKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self)
end;

procedure TFrmDesembolsoTmp.interes_ant;
...
procedure TFrmDesembolsoTmp.Actualiza_temp;
begin
...
procedure TFrmDesembolsoTmp.Elimina_temp;
begin
       if PermiteEliJud then
        begin
          ...
          begin
            ...
            Close;
            SQL.Clear;
            SQL.Add('delete from "col$pagocostastmp" where ID_SOLICITUD = :ID_SOLICITUD');
            ParamByName('ID_SOLICITUD').AsString := vIdSolicitud;
            ExecSQL;
            Close;
            Transaction.Commit;
...
procedure TFrmDesembolsoTmp.cambia_estado;
begin
        with DmSolicitud.IBSolicitud1 do
        begin
           Close;
...
procedure TFrmDesembolsoTmp.grupos(valor: boolean);
begin
...
procedure TFrmDesembolsoTmp.contabilizar_descuentos;
var    i :Integer;
begin
        for i := 0 to 1 do
        begin
...
function TFrmDesembolsoTmp.recupera_codigo(codigo: integer): string;
begin
    with DmSolicitud.IBSolicitud1 do
    begin
      if Transaction.InTransaction then
...
procedure TFrmDesembolsoTmp.actualiza_tabladesembolso(es_nuevo:Boolean);
...
procedure TFrmDesembolsoTmp.EDvalorcolocacionExit(Sender: TObject);
begin
        if vdesembolsoparcial then
        begin
...
function TFrmDesembolsoTmp.extrae_valor: currency;
var
valor :Currency;
vInteresesDto:Currency;
begin
        valor := 0;
        vInteresesDto := 0;

        if interes_anticipado then
          vInteresesDto := vInteresesDto + SimpleRoundTo((vDesembolso * vTasaNominal / 100 * DiasPago ) / 360,0);

        if vInteresVencido = 'A' then
          vInteresesDto := vInteresesDto + SimpleRoundTo(vDesembolso*(vTasaNominal/100)*vAmortizaInteres/360,0);

        valor := valor + vInteresesDto;
...
procedure TFrmDesembolsoTmp.verifica_creditos;
...
function TFrmDesembolsoTmp.es_juridico(id_persona:string;id_identificacion:integer): string;
begin
        with DmSolicitud.IBsolicitud2 do
        begin
          SQL.Clear;
...
function TFrmDesembolsoTmp.busca_tipo(id_persona: string;
  id_identificacion: integer): string;
begin
...
procedure TFrmDesembolsoTmp.EDpuntosKeyPress(Sender: TObject; var Key: Char);
begin
        EnterTabs(Key,Self);
end;

procedure TFrmDesembolsoTmp.EdFechaPagoChange(Sender: TObject);
begin
        edfechapagoexit(Self);
end;

procedure TFrmDesembolsoTmp.relacion_seguro;
begin
             with DmSolicitud.IBsolicitud3 do
             begin
               SQL.Clear;
...
//               SQL.Add('("col$relacionseguro".ID_COLOCACION, "col$relacionseguro".VALOR_SEGURO, "col$relacionseguro".FECHA_COLOCACION, "col$relacionseguro".ID_AGENCIA)');
               ...
               ParamByName('VALOR_SEGURO').AsCurrency := seguro(5);
               ParamByName('ID_AGENCIA').AsInteger := vagencia;
               ...
               if vFechaDesembolso = StrToDate('2008/02/29') then
                  vFechaDesembolso := vFechaDesembolso + 1;
...
procedure TFrmDesembolsoTmp.DBGridRealKeyPress(Sender: TObject;
  var Key: Char);
begin
        EnterTabs(Key,Self)
end;

procedure TFrmDesembolsoTmp.Eliminar1Click(Sender: TObject);
begin
        try
          if MessageDlg('Esta Seguro de Realizar la Transacci?n',mtinformation,[mbyes,mbno],0) = mryes then
...
procedure TFrmDesembolsoTmp.Modificar1Click(Sender: TObject);
begin
...
procedure TFrmDesembolsoTmp.Agregar1Click(Sender: TObject);
begin
        FrmAgregaGarantia := TFrmAgregaGarantia.Create(Self);
        with FrmAgregaGarantia do
        begin
...
function TFrmDesembolsoTmp.recupera_saldo(codigo, estado: Integer;
  tipo: string): Currency;
begin
        with IBSQL5 do
        begin
...
function TFrmDesembolsoTmp.verifica_fianza: boolean;
var     dias :Integer;
        id_persona :string;
begin
        Result := False;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              SQL.Add('where "col$colgarantias".ID_PERSONA = :ID_PERSONA AND');
              SQL.Add('"col$colgarantias".ID_IDENTIFICACION = :ID_IDENTIFICACION and');
              SQL.Add('("col$colocacion".ID_ESTADO_COLOCACION NOT IN(2,7,8,5,4))');
              ...
              while not Eof do
              begin
                if fianzadescuento(FieldByName('ID_COLOCACION').AsString) = False then
                begin
                  dias := obtenerdeudahoy1(Agencia,FieldByName('ID_COLOCACION').AsString,IBSQL8).Dias;
                  if dias >= vMaxDias then
                  begin
                    Result := True;
                    Break;
                  end
                  else if (dias > 0) and (EdNumeroIdentificacion.Text <> FieldByName('ID_PERSONA').AsString)  then
                  begin
                    if vFianza(FieldByName('ID_COLOCACION').AsString) = False then
                    begin
                      Result := True;
...
procedure TFrmDesembolsoTmp.ChedadClick(Sender: TObject);
begin
        Chedad.Checked := es_mayor;
end;

procedure TFrmDesembolsoTmp.inserta_marcas;
begin
             with DmSolicitud.IBsolicitud3 do
             begin
               SQL.Clear;
...
procedure TFrmDesembolsoTmp.CHcredividaClick(Sender: TObject);
begin
        CHcredivida.Checked := vId_vida;
end;

procedure TFrmDesembolsoTmp.BTJudCasClick(Sender: TObject);
begin
        LimpiaInformacion;
        DBGridJudicial.Visible := True;
        DBGridJudicial.Refresh;
        Grupoinformacion.Caption := 'Descuentos de Cr?ditos Judicial y Castigado';
end;

function TFrmDesembolsoTmp.Capitalizacion: Currency;
var
    vVecesAportes,
    vVecesAhorros,
    vPorcCredito,
...
procedure TFrmDesembolsoTmp.RevisaHonorarios;
begin
        with IBSQL1 do begin
          ...
          Close;
          SQL.Clear;
          SQL.Add('update "col$pagocostastmp" set ID_COLOCACION_DES = :ID_COLOCACION_DES');
          SQL.Add('where ID_SOLICITUD = :ID_SOLICITUD');
          ParamByName('ID_SOLICITUD').AsString := vIdSolicitud;
          ParamByName('ID_COLOCACION_DES').AsString := VidColocacion;
          ExecQuery;
          Close;
          Transaction.Commit;
        end;
...
procedure TFrmDesembolsoTmp.InfDesembolso;
begin
        with DmSolicitud.IBsolicitud3 do
        begin
          if Transaction.InTransaction then
          ...
          Close;
          SQL.Clear;
          SQL.Add('insert into "col$desembolso" (ID_AGENCIA,ID_COLOCACION,HORA_DESEMBOLSO)');
          SQL.Add('values (:ID_AGENCIA,:ID_COLOCACION,:HORA_DESEMBOLSO)');
          ParamByName('ID_AGENCIA').AsInteger := Agencia;
          ParamByName('ID_COLOCACION').AsString := VidColocacion;
          ParamByName('HORA_DESEMBOLSO').AsTime :=  fHoraActual;
          ExecSQL;
          Transaction.Commit;
        end;
end;

function TFrmDesembolsoTmp.vFianza(vId_Colocacion: string): boolean;
begin
        Result := False;
        with IBSQL1 do
        begin
          ...
          SQL.Add('"col$fianza"');
          SQL.Add('WHERE');
          SQL.Add('("col$fianza".ID_SOLICITUD = :ID_SOLICITUD) AND ');
          SQL.Add('("col$fianza".ID_COLOCACION = :ID_COLOCACION)');
          ParamByName('ID_SOLICITUD').AsString := DBsolicitud.Text;
...
procedure TFrmDesembolsoTmp.BTBancoldexClick(Sender: TObject);
begin
        LimpiaInformacion;
        DBGridBancoldex.Visible := True;
        GrupoInformacion.Caption := 'Informaci?n Bancoldex';
end;

function TFrmDesembolsoTmp.FianzaDescuento(vId_Colocaicon: string): boolean;
begin
        with IBSQL1 do
        begin
          if Transaction.InTransaction then
          ...
          SQL.Add('"gen$infcrediticia"');
          SQL.Add('WHERE');
          SQL.Add('(ID_SOLICITUD = :ID_SOLICITUD) AND ');
          SQL.Add('(ID_COLOCACION = :ID_COLOCACION)');
          ParamByName('ID_COLOCACION').AsString := vId_Colocaicon;
          ParamByName('ID_SOLICITUD').AsString := DBsolicitud.Text;
          ExecQuery;
...
procedure TFrmDesembolsoTmp.BTfngClick(Sender: TObject);
begin
        LimpiaInformacion;
        PCuenta.Visible := True;
        GrupoInformacion.Caption := 'Informaci?n Cuentas Adicionales';
