...
uses
  ...
  IBQuery, FR_Class, FR_DSet, FR_DBSet, DBClient, frexpimg,sdXmlDocuments, JclFileUtils, StrUtils;
...
type
  TfrmMain = class(TForm)
    ...
    SolicitudesyColocacion1: TMenuItem;
    AsociadosnoActualizados1: TMenuItem;
    N7: TMenuItem;
    ActualizacindeAsociadosporEmpleado1: TMenuItem;
    Siplaft1: TMenuItem;
    ConsolidacinSiplaft1: TMenuItem;
    ConsolidacinProductos1: TMenuItem;
    ...
    procedure MovimientoDiario1Click(Sender: TObject);
    procedure SolicitudesyColocacion1Click(Sender: TObject);
    procedure AsociadosnoActualizados1Click(Sender: TObject);
    procedure ActualizacindeAsociadosporEmpleado1Click(Sender: TObject);
    procedure ConsolidacinSiplaft1Click(Sender: TObject);
    procedure ConsolidacinProductos1Click(Sender: TObject);
  private
...
uses unitLogin, unitdmColocacion, IniFiles,
     ...
     UnitCambios, UnitInformeColocacionFiltrado, UnitBalanceGeneral,
     UnitInformeAuxiliar, UnitInformeDiarioCaja,ShellApi;
...
begin
  ...
  try
    ...
    free;
        ...
        frmLogin.EdRole.Text := 'FINANCIERO';
...
begin
        Self.Caption := 'Módulo Financiero - Build:' + ApplicationVersion +' - ' + DBserver;
        if SalirMal then
           Self.Close;
end;
...
begin
        FrmInfAsociados := TFrmInfAsociados.Create(Self);
        FrmInfAsociados.ShowModal;
end;
...
procedure TfrmMain.MovimientoDiario1Click(Sender: TObject);
...
begin
  ...
  Result := FInfo.FileVersion;
end;

procedure TfrmMain.SolicitudesyColocacion1Click(Sender: TObject);
var
  Parametros : string;
begin
  Parametros := 'SYSDBA';
  Parametros := Parametros + ' ' + 'masterkey';
  ...
  ShellExecute(self.Handle, 'open', PChar(ruta1+'\reportes\RepSolicitudCredito.exe'),Pchar(Parametros), '' ,SW_NORMAL);

end;

procedure TfrmMain.AsociadosnoActualizados1Click(Sender: TObject);
var
  Parametros : string;
begin
  Parametros := 'SYSDBA';
  ...
  Parametros := Parametros + ' ' + Nit;
  Parametros := Parametros + ' ' + AnsiReplaceStr(UnitGlobales.extrae_empleado,' ','#');
  ShellExecute(self.Handle, 'open', PChar(ruta1+'\reportes\RepAsociadoNoActualizado.exe'),Pchar(Parametros), '' ,SW_NORMAL);
end;

procedure TfrmMain.ActualizacindeAsociadosporEmpleado1Click(
  Sender: TObject);
var
  Parametros : string;
begin
  ...
  Parametros := Parametros + ' ' + Nit;
  Parametros := Parametros + ' ' + AnsiReplaceStr(UnitGlobales.extrae_empleado,' ','#');
  ShellExecute(self.Handle, 'open', PChar(ruta1+'\reportes\RepActualizaXEmpleado.exe'),Pchar(Parametros), '' ,SW_NORMAL);
end;

procedure TfrmMain.ConsolidacinSiplaft1Click(Sender: TObject);
var
  Parametros : string;
begin
  Parametros := DBAlias;
  Parametros := Parametros + ' ' + DBPasabordo;
  ShellExecute(self.Handle, 'open', PChar(ruta1+'\siplaft\PlanoSiplaft\PlanoSiplaft.exe'),Pchar(Parametros), '' ,SW_NORMAL);
end;

procedure TfrmMain.ConsolidacinProductos1Click(Sender: TObject);
var
  Parametros : string;
begin
  Parametros := DBAlias;
  Parametros := Parametros + ' ' + DBPasabordo;
  ShellExecute(self.Handle, 'open', PChar(ruta1+'\siplaft\PlanoSiplafProductos\PlanoProducto.exe'),Pchar(Parametros), '' ,SW_NORMAL);
end;
