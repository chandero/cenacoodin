...
procedure TfrmNormalizar.btnNormalizarClick(Sender: TObject);
begin
        if (MessageDlg('Seguro de Normalizar la Colocación', mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
        begin
...
procedure TfrmNormalizar.FormShow(Sender: TObject);
var
  _vFecha :TDate;
begin
...
procedure TfrmNormalizar.FormCreate(Sender: TObject);
begin
        IBTransaction1.DefaultDatabase := dmGeneral.IBDatabase1;
        IBTransaction1.StartTransaction;
end;

procedure TfrmNormalizar.btnCerrarClick(Sender: TObject);
begin
...
procedure TfrmNormalizar.DBGplanDrawColumnCell(Sender: TObject;
...
procedure TfrmNormalizar.DBGNplanDrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
          if CDSNplanPAGADA.Value = False then
...
procedure TfrmNormalizar.CDSplanPAGADAGetText(Sender: TField;
...
procedure TfrmNormalizar.CDSNplanPAGADAGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
    if (Sender.Value = False) Then Text := 'No'
    else Text := 'Si';
