unit UnitLiquidacionCarteraJudicial_07072007;
      ...
      begin
       ...
       if vCuotaTipo = 'V' then
        if (CapitalaPagar > 0) and (InteresaPagar > 0) then
         begin
           if (FechaTabla <= NuevaFechaCapital) and (InteresaPagar <= vTotalInteres) and (SaldoTabla <= (vSaldoActual - vAbonocapital)) then
            begin
              IBConsulta.sql.Clear;
              IBConsulta.sql.Add('update "col$tablaliquidacion" set ');
              IBConsulta.sql.Add('"col$tablaliquidacion"."PAGADA" = 1');
...
///**Genero el consecutivo
    ...
    with IBQuery do
     try
      ...
      New(AR);
      AR^.CuotaNumero := 0;
      AR^.CodigoPuc   := '615035000000000000';
      AR^.FechaInicial := fFechaActual;
      AR^.FechaFinal   := fFechaActual;
      AR^.Dias         := 0;
      AR^.Tasa         := 0;
