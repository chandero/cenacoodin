...
begin
       ...
       IBSQL1.Close;
       IBSQL1.SQL.Clear;
       IBSQL1.SQL.Add('DELETE FROM CON$SALDOSCUENTA WHERE ID_AGENCIA = :ID_AGENCIA and MES = :MES');
       IBSQL1.ParamByName('ID_AGENCIA').AsInteger := DBLCBAgencias.KeyValue;
       IBSQL1.ParamByName('MES').AsInteger := CBMeses.ItemIndex + 1;
       try
         IBSQL1.ExecQuery;
       except
         JvPf.Free;
       ...
       begin
         ...
         if Line <> '' then begin
           ...
           Agencia := StrToInt(Tipo.Strings[0]);
           Codigo := Trim(Tipo.Strings[1]);
           Completo := '000000000000000000';
           Insert(Codigo,Completo,0);
           Codigo := Completo;
           Mes := StrToInt(Trim(Tipo.Strings[2]));
           Debitos := StrToCurr(Trim(Tipo.Strings[3]));
           Creditos := StrToCurr(Trim(Tipo.Strings[4]));
