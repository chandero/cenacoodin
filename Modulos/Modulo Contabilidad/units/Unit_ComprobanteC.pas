...
uses UnitdmGeneral, digitacuenta, digitacuentaC, UnitVistaPreliminar,
...
function TfrmComprobanteC.gettipo_comp : integer;
begin
        result:= vtipo_comprobante;
end;

procedure TfrmComprobanteC.settipo_comp(nvalor:integer);
begin
             ...
             if ARecord^.credito > 0 then
               if ARecord^.gmf then
                 if (ARecord^.codigo = '110505000000000000') or
                    ...
                    (ARecord^.codigo = '111005090000000000') then
   ...
   with list do
     ...
     if not vmodificar then begin
       ...
       if (TAPO+TPAGOS+TPAGOSINT) > 0 then begin
          ...
          xsauxiliar.Cells [2,XSauxiliar.RowCount-1] := '';
          xsauxiliar.Cells [3,XSauxiliar.RowCount-1] := formatcurr ('#,##0.00',0.00);
          xsauxiliar.Cells [4,XSauxiliar.RowCount-1] := formatcurr ('#,##0.00', SimpleRoundTo((TAPO / 1000 ) * 4,0)+SimpleRoundTo((TPAGOS / 1000 ) * 4,0)+SimpleRoundTo((TPAGOSINT / 1000 ) * 4,0));
          xsauxiliar.Cells [5,XSauxiliar.RowCount-1] := '';
       end;
       ...
       if (TAHOV+TAHOC+TAPO+TPAGOS+TPAGOSINT) > 0 then begin
          ...
          xsauxiliar.Cells [2,XSauxiliar.RowCount-1] := '';
          xsauxiliar.Cells [3,XSauxiliar.RowCount-1] := formatcurr ('#,##0.00', SimpleRoundTo((TAHOV / 1000 ) * 4,0)+SimpleRoundTo((TAHOC / 1000 ) * 4,0)+SimpleRoundTo((TAPO / 1000 ) * 4,0)+SimpleRoundTo((TPAGOS / 1000 ) * 4,0)+SimpleRoundTo((TPAGOSINT / 1000 ) * 4,0));
          xsauxiliar.Cells [4,XSauxiliar.RowCount-1] := formatcurr ('#,##0.00', 0.00);
          xsauxiliar.Cells [5,XSauxiliar.RowCount-1] := '';
          vtotal_debito := vtotal_debito + SimpleRoundTo((TAHOV / 1000 ) * 4,0)+SimpleRoundTo((TAHOC / 1000 ) * 4,0)+SimpleRoundTo((TAPO / 1000 ) * 4,0)+SimpleRoundTo((TPAGOS / 1000 ) * 4,0)+SimpleRoundTo((TPAGOSINT / 1000 ) * 4,0);
          vtotal_credito := vtotal_credito + SimpleRoundTo((TAHOV / 1000 ) * 4,0)+SimpleRoundTo((TAHOC / 1000 ) * 4,0)+SimpleRoundTo((TAPO / 1000 ) * 4,0)+SimpleRoundTo((TPAGOS / 1000 ) * 4,0)+SimpleRoundTo((TPAGOSINT / 1000 ) * 4,0);
          edittotaldebito.Text := currtostr(vtotal_debito);
          edittotalcredito.Text := currtostr(vtotal_credito);
       end;
     end;
...
procedure TfrmComprobanteC.dblcbtipo_compEnter(Sender: TObject);
begin
        dblcbtipo_comp.KeyValue := 1;
end;

procedure TfrmComprobanteC.dblcbtipo_compExit(Sender: TObject);
begin
...
procedure TfrmComprobanteC.DblcbAgenciaEnter(Sender: TObject);
begin
        dblcbagencia.KeyValue := Agencia;
end;

procedure TfrmComprobanteC.DblcbAgenciaExit(Sender: TObject);
begin
          ...
          begin
                ...
                if (auxiliar <> 0) then
                 begin
                    ...
                    if FieldByName('ID_CUENTA').AsString = '' then
                       Arecord^.nocuenta:= 0
                    else
...
function TfrmComprobanteC.GrabarCaptacion(Cuenta: integer; ValorD,
  ValorC: Currency): Boolean;
var Valor:Currency;
    I:Integer;
    Dg:Integer;
begin
              ...
              begin
               ...
               IBPagar.ParamByName('DESCRIPCION_MOVIMIENTO').AsString := format('%.50s',[editdescripcion.Text]);
...
{              IBPagar.Close;
              ...
              IBPagar.ParamByName('NUMERO_CUENTA').AsInteger := Cuenta;
              IBPagar.ParamByName('DIGITO_CUENTA').AsInteger := Dg;
              IBPagar.ParamByName('MES').AsString := Format('%.2d',[MonthOf(StrToDate(FechaDia.Caption))]);
              if ValorC > 0 then begin
              IBPagar.ParamByName('DEBITO').AsCurrency := Valor;
              IBPagar.ParamByName('CREDITO').AsCurrency := 0;
              end
              else
              begin
              IBPagar.ParamByName('DEBITO').AsCurrency := 0;
              IBPagar.ParamByName('CREDITO').AsCurrency := Valor;
              end;
              try
                IBPagar.ExecQuery;
                if IBPagar.RowsAffected < 1 then
...
//      colocacion := InttoBoolean(FieldByName('ESCOLOCACION').AsInteger);
        ...
        if colocacion then
         begin
           MessageDlg('No se aplicaran cambios a la colocaci?n',mtInformation,[mbok],0);
         end;
      end
     else
      begin
...
begin
        ...
        xsauxiliar.Cols[2].Text  := 'No.Cuenta/Cr?dito';
