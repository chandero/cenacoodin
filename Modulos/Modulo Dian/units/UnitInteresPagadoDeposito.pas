unit UnitInteresPagadoDeposito;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, DB, DBClient, IBCustomDataSet, IBQuery, Grids, DBGrids,
  StdCtrls, Buttons, ExtCtrls, UnitDmGeneral, DataSetToExcel, StrUtils, Math;

type
  TfrmInteresPagadoDeposito = class(TForm)
    Panel1: TPanel;
    btnProcesar: TBitBtn;
    btnCerrar: TBitBtn;
    DBGrid1: TDBGrid;
...
procedure TfrmInteresPagadoDeposito.btnProcesarClick(Sender: TObject);
var
        Tp,Id,Dg,Mn :string;
...
begin
        ...
        begin
          Close;
          SQL.Clear;
          SQL.Add('SELECT gp1.ID_IDENTIFICACION, gp1.ID_PERSONA,  SUM(ce1.VALOR_DEBITO) AS INTERES  FROM "cap$extracto" ce1');
          ...
          SQL.Add('WHERE ce1.ID_TIPO_CAPTACION IN (2,3,4) AND ce1.FECHA_MOVIMIENTO BETWEEN :FECHA_INICIAL AND :FECHA_FINAL');
          SQL.Add('AND ce1.ID_TIPO_MOVIMIENTO IN (7)');
        ...
        save.FileName := 'InteresPagadoDepositos';
...
procedure TfrmInteresPagadoDeposito.btnCerrarClick(Sender: TObject);
begin
        Close;
end;
