...
function CalculoFechaMes(FechaActual:TDate;Dias:Integer):TDate;
...
//               TasaMora := ValorMora;
            ...
            if (Clasificacion = 1) or (Clasificacion = 2) then
             begin
...
//               TasaMora := ValorMora;
            ...
            if ((Int(FechaArranque) = Int(AR^.FechaInicial)) and (Mes=3) and (Dia=1) and (AF^.Devuelto=False) and (AF^.Vencida=False)) then
                ...
                Bisiesto := False;
            AR^.Dias := DiasEntreFechas(AF^.FechaInicial,AF^.FechaFinal,FechaDesembolso,bisiesto);
            if AF^.Vencida then
               AR^.Tasa := TasaMora
            else
               AR^.Tasa := TasaLiquidar;
            ...
            try
              ...
              for J := 0 to (FechasLiq.Count - 1) do
               begin
                 ...
                 else if TipoInteres = 1 then begin
                   TasaEfectiva := BuscoTasaEfectivaMaximaDtf(FechaPagoI);
                   TasaLiquidar := TasaEfectiva;
                 end
                 else begin
