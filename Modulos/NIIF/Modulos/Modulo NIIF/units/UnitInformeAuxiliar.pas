...
type
  TfrmInformeAuxiliares = class(TForm)
    ...
    IBQuery1DETALLECOMP: TMemoField;
    ReporteAuxiliarGrafico: TprReport;
...
begin
        With IBQuery1 do
         begin
           ...
           SQL.Add('CON$AUXILIAR.DESCRIPCION AS DESCRIPCION,');
           ...
           SQL.Add('CON$COMPROBANTE.DESCRIPCION AS DETALLECOMP,');
...
begin
    ...
    if (VarName = 'BENEFICIARIO') then
     ...
     if (VarName = 'IDENTIFICACION') then
     begin
       Identificacion := trim(IBQuery1.FieldByName('ID_PERSONA').AsString);
       _vSetAsString(Value, Identificacion);
       IsProcessed := True;
     end;

    if (VarName = 'DESCRIPCION') then
    begin
        if (trim(IBQuery1.FieldByName('DESCRIPCION').AsString) = '') then
        Detalle := IBQuery1.FieldByName('DETALLECOMP').AsString
        else
        Detalle := IBQuery1.FieldByName('DESCRIPCION').AsString;
        _vSetAsString(Value, Detalle);
        IsProcessed := True;
    end;
