...
uses
  ...
  IdTCPClient, IdTime, ExtCtrls, DateUtils, IBSQL, UnitInformeBalance, UnitLibroCajaDiario, UnitLibroRMayorYBalance,
  UnitLibroBalance, UnitEquivalencia;
...
type
  TfrmMain = class(TForm)
    ...
    BalanceGeneral2: TMenuItem;
    MantenimientoEquivalenciasNIIF1: TMenuItem;
    SincronizacindeCuentas1: TMenuItem;
    N15: TMenuItem;
    N18: TMenuItem;
    ...
    procedure EstadodeIngresosyGastos1Click(Sender: TObject);
    procedure BalanceGeneral1Click(Sender: TObject);
    procedure BalanceGeneral2Click(Sender: TObject);
...
uses unitLogin, IniFiles,
     ...
     UnitNotasCoop, UnitProcesoCierreAno, UnitCierreAnual,
...
{        IdTime1.Host := DBserver;
  ...
  TheGraphic.LoadFromFile(Ruta+'\'+logofondo);
  dmGeneral.Free;
end;
...
begin
        Self.Caption := 'Módulo NIIF v1.0 - ' + IntToStr(DBAnho) + ' - ' + DBserver + ':' + DBPath + DBName;
        if SalirMal then
           Self.Close;
end;
...
var frmComprobante:TfrmComprobanteC;
begin
        frmComprobante := TfrmComprobanteC.Create(Self);
...
var frmBalanceGeneral:TfrmMayorYBalance;
begin
          frmBalanceGeneral := TfrmMayorYBalance.Create(Self);
          frmBalanceGeneral.ShowModal;
end;

procedure TfrmMain.BalanceGeneralDetallado1Click(Sender: TObject);
var frmBalanceGralDetallado:TfrmBalance;
begin
        frmBalanceGralDetallado := TfrmBalance.Create(Self);
        frmBalanceGralDetallado.ShowModal;
end;

procedure TfrmMain.LibrosAuxiliares1Click(Sender: TObject);
var frmInformeAuxiliares :TfrmAuxiliar;
begin
    frmInformeAuxiliares := TfrmAuxiliar.Create(Self);
...
var
 FrmNotas : TFrmNotasCoop;
begin
        FrmNotas := TFrmNotasCoop.Create(Self);
        FrmNotas.ShowModal;
end;
...
procedure TfrmMain.EstadodeIngresosyGastos1Click(Sender: TObject);
var
  frmInformeEstadoIngresosGastos: TfrmInformeEstadoIngresosGastos;
begin
     frmInformeEstadoIngresosGastos := TfrmInformeEstadoIngresosGastos.Create(self);
     frmInformeEstadoIngresosGastos.ShowModal;
end;
...
begin
  frmInformeBalance := TfrmInformeBalance.Create(self);
  frmInformeBalance.ShowModal;
end;

procedure TfrmMain.BalanceGeneral2Click(Sender: TObject);
var
   frmLibroBalanceGeneral : TfrmLibroBalanceGeneral;
begin
   frmLibroBalanceGeneral := TfrmLibroBalanceGeneral.Create(self);
   frmLibroBalanceGeneral.ShowModal;
end;

procedure TfrmMain.MantenimientoEquivalenciasNIIF1Click(Sender: TObject);
var
...
procedure TfrmMain.SincronizacindeCuentas1Click(Sender: TObject);
var
  frmSincronizarPlanCuentasNIIF : TfrmSincronizarPlanCuentasNIIF;
begin
        frmSincronizarPlanCuentasNIIF := TfrmSincronizarPlanCuentasNIIF.Create(self);
        frmSincronizarPlanCuentasNIIF.ShowModal;

end;

procedure TfrmMain.MigracindeNotasColgaap1Click(Sender: TObject);
var
...
procedure TfrmMain.ConsultarComprobantes1Click(Sender: TObject);
var
